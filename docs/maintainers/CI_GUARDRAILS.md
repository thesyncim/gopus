...
- The unsupported-controls smoke and required DRED parity sweep stay separate so quarantine API exposure remains a small gate while libopus-backed DRED carried-payload, bootstrap, and bookkeeping seams still block the aggregate correctness check. The decoder audio numerical matrix and real-model RDOVAE/PitchDNN/conceal-analysis oracles remain in `make test-unsupported-controls-parity-experimental` until their Linux matrix is green.
