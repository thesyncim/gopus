...
- `test-linux-libopus-control-oracle`: `make test-libopus-control-oracle`
...
- Internally split into parallel jobs (`test-linux-parity`, `test-linux-race`, `test-linux-provenance`, `test-linux-flake`, `test-linux-fuzz-smoke`, `test-linux-consumer-smoke`, `test-linux-libopus-control-oracle`, `test-linux-dnn-blob-parity`, `test-linux-dred-tag`, `test-linux-qext-parity`, `test-linux-unsupported-controls`, `test-linux-unsupported-controls-parity`) and aggregated by `test-linux`.
- This keeps parity/race/provenance/fuzz coverage intact while removing serialized Linux checks from a single job.
- The broad encoder/decoder public control oracle is explicitly built only with `gopus_libopus_oracle`, runs through `make test-libopus-control-oracle`, and fails on skipped helper coverage. Ordinary package tests keep the helper compiler/import path out of the build graph.
