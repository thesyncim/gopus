...
- Runs on markdown-only PRs too, so optional-extension docs changes still pass through the same static/docs contract checks, default public API surface checks, public API round-trip/PLC/SILK smoke checks, constructor/API-abuse validation checks, build-safety checks, wrapper safety checks, core hot-path allocation checks, default encoder encode-path/state/routing checks, default single-stream and multistream control checks, hybrid packet decode/range/band-state checks, CELT band/PVQ encode-decode checks, CELT FFT butterfly checks, CELT tonality/MDCT checks, streaming reader/writer checks, streaming reader/writer data-flow and end-to-end stream checks, Ogg reader/writer contract and flow checks, Ogg page primitive/packet/metadata, projection demixing, and internal integration checks, default decoder decode-path/packet-state/FEC/provided-packet FEC/LBRR recovery checks, default multistream constructor/round-trip/sample-format/PLC, internal encoder mapping/packet assembly/policy, internal decoder mapping/conversion/full packet decode integration, ambisonics/projection mapping, package round-trip/channel-isolation, and decoder duration checks, rangecoder encode/decode round-trip checks, default-build optional-control semantics/quarantine checks, default packet-extension dormancy/framing checks, default parser-envelope checks, and root repacketizer libopus fixture/envelope checks.
