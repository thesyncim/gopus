...
- The supported QEXT tag gate builds a separate `tmp_check/opus-1.6.1-qext` libopus tree with `--enable-qext`, runs no-skip packet-extension parity plus packet/repacketizer/self-delimited extension preservation coverage under `gopus_qext`, and keeps default-build QEXT controls absent with packet-extension plumbing compile-time dormant unless the tag is set.
