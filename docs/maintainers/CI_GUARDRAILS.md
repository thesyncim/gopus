...
- The supported DRED tag gate carries standalone DRED wrapper lifecycle/no-allocation coverage, a standalone libopus parse/decode proof, and the libopus-backed narrow SILK wideband 20 ms encoder carried-payload/primary-budget proof directly under `gopus_dred`; the unsupported-controls smoke and required DRED parity sweep stay separate so quarantine API exposure remains a small gate while bootstrap and bookkeeping seams also block the aggregate correctness check. The wider carried-payload matrix, decoder audio numerical matrix, and real-model RDOVAE/PitchDNN/conceal-analysis oracles remain in `make test-unsupported-controls-parity-experimental` until their Linux matrix is green.
