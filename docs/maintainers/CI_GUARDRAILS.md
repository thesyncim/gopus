...
- Runs on markdown-only PRs too, so optional-extension docs changes still pass through the same static/docs contract checks, default public API surface checks, constructor/API-abuse validation checks, wrapper safety checks, core hot-path allocation checks, default encoder encode-path/state/routing checks, default single-stream and multistream control checks, streaming reader/writer checks, streaming reader/writer data-flow and end-to-end stream checks, Ogg reader/writer contract and flow checks, Ogg page primitive/packet/metadata, projection demixing, and internal integration checks, default decoder decode-path/packet-state/FEC/provided-packet FEC/LBRR recovery checks, default multistream constructor/round-trip/sample-format/PLC, internal encoder mapping/packet assembly/policy, internal decoder mapping/conversion, ambisonics/projection mapping, and decoder duration checks, default-build optional-control semantics/quarantine checks, default packet-extension dormancy/framing checks, default parser-envelope checks, and root repacketizer libopus fixture/envelope checks.
