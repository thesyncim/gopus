...
- The docs contract gate pins default public API shape, wrapper safety behavior, single-stream and multistream control behavior, streaming reader behavior, decoder packet-state, normal FEC, provided-packet FEC, and multistream decoder duration behavior, unsupported-control quarantine, packet-extension dormancy/framing, parser-envelope behavior, and root repacketizer libopus fixture/envelope behavior before the heavier parity lanes run.
