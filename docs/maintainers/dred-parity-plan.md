...
- these decoder parity claims are seam-specific and libopus-backed; the current mono explicit/live decoder numerical matrix and the selected 48 kHz CELT cached stereo first-loss seam are now required in `make test-unsupported-controls-parity`, while broader stereo, multistream, and packet coverage remains separate work unless explicitly covered by green parity tests
...
- the required supported DRED tag gate now carries standalone DRED wrapper lifecycle/no-allocation, libopus parse/decode/process metadata coverage, real-packet standalone process state/feature parity, standalone recovery scheduling parity, decoder cached recovery bookkeeping parity, the SILK wideband 20/40/60 ms mono and 20 ms stereo encoder carried-payload proofs, the Hybrid fullband 20 ms mono carried-payload-only proof, and the Hybrid fullband 20 ms stereo carried-payload/packet-envelope proof, while the unsupported-controls gate covers the green quarantine/core and selected parity surface: API exposure, parser availability, those same encoder seams, internal converter/payload/basic-analysis seams, the real-model PitchDNN and RDOVAE encoder oracles, the conceal-analysis oracle, 48 kHz runtime bootstrap checks, the current mono decoder explicit/live numerical matrix, exercised 16 kHz Hybrid mono live-sequence seams, and the 48 kHz CELT cached stereo first-loss live-sequence seam
- Hybrid mono 20/40 ms packet-envelope exactness and Hybrid stereo 40 ms packet-envelope/primary-frame exactness are opt-in diagnostics until the Linux amd64 packet-shape drift is closed; broader stereo/multistream decoder coverage and support-surface graduation remain open
...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms mono and 20 ms stereo carried-payload seams, the Hybrid fullband 20 ms mono carried-payload-only seam, and the Hybrid fullband 20 ms stereo carried-payload/packet-envelope seam pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the current Hybrid carried-packet tests cover packet length, offset, DRED payload, and primary-frame sizes only on the green 20 ms stereo seam; Hybrid mono 20/40 ms packet-envelope exactness, Hybrid stereo 40 ms packet-envelope/primary-frame exactness, Hybrid stereo primary-frame byte exactness, broader Hybrid packet-shape matrices, and SILK primary-frame byte exactness still need separate libopus-backed coverage before support docs can claim them
...
- the OSCE BWE forward-pass (`bbwenet_process_frames` via the public `osce_bwe(...)` entry point) now has a libopus-backed bounded-divergence parity probe in `decoder_osce_bwe_forward_pass_libopus_parity_test.go::TestOSCEBWEForwardPassMatchesLibopusBitExact`. The probe is **not** a bit-exact contract: libopus uses `tansig_approx` / `sigmoid_approx` / `celt_exp` / `celt_log2` for the BBWENET GRU + AdaConv stages while the pure-Go runtime uses `dnnmath.SigmoidApprox` / `dnnmath.TanhApprox` + `math.Exp` / `math.Sin`, and the libopus output is int16-quantised with a 21-sample delay buffer (`OSCE_BWE_OUTPUT_DELAY`). On a 1 kHz sinusoid the current divergence is `maxAbs ~ 0.015`, `rms ~ 0.0018` (float-PCM domain in [-1, 1]). The lmspec block of the feature extractor agrees to `~ 1e-5`; the instafreq cross-power features show large divergence on the very first frame because both sides prime `last_spec[2*k] = 1e-9` and small-amplitude near-DC bins are dominated by that prime through the `re1*re2 + im1*im2` cross-power. Tightening either tolerance requires porting libopus's exact `tansig_approx` / `sigmoid_approx` / `celt_exp` math into `internal/dnnmath` and adding the int16 output-delay path in `decoder_osce_bwe_apply.go` (the runtime-level wiring around the cross-fade is in `decoder_osce_bwe_crossfade.go` but currently does not flow through `osce_bwe`'s int16 quantisation).
...
- Side effect of the SILK stereo 40 ms fix on Hybrid 40 ms stereo: libopus splits Hybrid mode frames > 20 ms into 20 ms sub-frames (`opus_encoder.c:1698`) before calling `dred_compute_latents()`, so each Hybrid sub-frame has frame_size16k=320 and uses only ONE iter of the buggy `pcm += process_size` loop, meaning libopus Hybrid 40 ms stereo never triggers the under-advance. Gopus currently calls `processDREDLatents` once per 40 ms Hybrid frame and now under-advances within that single call to match libopus SILK 40 ms stereo behavior. As a result the opt-in `TestEncoderCarriedDREDPayloadMatchesLibopusHybridFullband40msStereo` diagnostic now diverges (gopus 169 B vs libopus 147 B); it was previously matching by accidental compensation between gopus's correct per-channel advance and the absence of frame-splitting. The proper fix is to mirror libopus's `nb_frames` split path so Hybrid > 20 ms also makes one `processDREDLatents` per sub-frame; until that lands, Hybrid 40 ms stereo packet-envelope exactness stays a known-divergent diagnostic.
...
- decoder-level parity beyond the current mono seams, exercised 16 kHz Hybrid mono seams, and the selected 48 kHz CELT cached stereo first-loss seam, especially broader stereo/multistream coverage, broader packet coverage, and the final supported-surface decisions for what graduates from quarantine
- `SetDNNBlob(...)` only validates the blob and retains the model-loaded flags (`pitchDNNLoaded`, `plcModelLoaded`, `farganModelLoaded`, `osceModelsLoaded`, `osceBWEModelLoaded`) plus, in tag-gated builds, the eligible 16 kHz / 48 kHz one- or two-channel DRED neural-runtime arming inside `ensureDREDNeuralConcealmentRuntime()`; OSCE LACE/NoLACE postfilter, OSCE BWE upsampler, and any FARGAN-as-ordinary-PLC seam stay un-wired in every build, while Hybrid/SILK stereo, multistream, and non-16-or-48 kHz DRED neural concealment remain gated off independently of blob readiness
...
- broader live-oracle adoption beyond the covered mono cached seams and selected 48 kHz CELT stereo first-loss seam; some cached/live tests still compare against the explicit `opus_decoder_dred_decode_float()` helper, and broader stereo/multistream paths plus wider packet matrices still need migration to live-sequence coverage where those surfaces become supported
...
The 48 kHz CELT cached stereo first-loss seam now follows that shape and is
required in `make test-unsupported-controls-parity` through
`TestDecoderCachedStereoDREDDecodeMatchesLiveSequenceOracle`.

What is green today:

- `dredNeuralConfigEligible()` admits 16 kHz / 48 kHz one- or two-channel
  single-stream decoders, but the green stereo audio claim is still seam-specific.
...
- The CELT wrapper mirrors channel-0 neural state into channel 1 and interleaves
  duplicated mono PCM for the selected 48 kHz CELT stereo cached first-loss seam.
- The new live-sequence parity test verifies libopus duplicated L/R PCM plus
  retained PLC, FARGAN, and CELT bridge state within the existing stereo DRED
  tolerances.

What remains open:

- 48 kHz CELT stereo second-loss, resumed-good-packet, broader frame-size, and
  broader packet-shape coverage.
- Hybrid/SILK stereo decoder DRED, where the remaining work is still in the SILK
  mid/lowband hook path (`beginHybridDREDLowbandHook`,
  `beginDREDRawMonoGoodFrameCapture`, `refreshDREDHistoryFromHybridDecoder`, and
  `advanceHybridDREDLowbandState`) rather than a separate per-channel neural
  runtime.
- Multistream decoder DRED, which still keeps standalone DRED on a
  cache/timing-only path (see the existing multistream optional-state hardening
  entry above).
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-payload seams, the Hybrid fullband 20 ms mono carried-extension payload-only seam, and the Hybrid fullband 20 ms stereo carried-extension packet seam, while Hybrid mono 20/40 ms packet-envelope exactness, Hybrid stereo 40 ms packet-envelope/primary-frame exactness, broader packet-shape coverage, primary-frame byte exactness, and final support-surface decisions remain open
