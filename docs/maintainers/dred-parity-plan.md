...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms mono and 20 ms stereo carried-payload/primary-frame seams, 20 ms primary-budget seam, and Hybrid fullband 20/40 ms mono plus 20 ms stereo carried-payload/primary-frame seams pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the current Hybrid carried-packet tests cover packet length, offset, DRED payload, and primary-frame byte exactness on the green libopus-backed seams; broader Hybrid packet-shape matrices still need separate libopus-backed coverage before support docs can claim them
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-extension primary-frame seams, Hybrid fullband 20/40 ms mono plus 20 ms stereo carried-extension primary-frame/payload exactness, and primary DRED budget reservation on the covered SILK case, while broader packet-shape coverage and final support-surface decisions remain open
