...
- the required unsupported-controls gate now covers the green quarantine/core and selected non-decoder-audio parity surface: API exposure, standalone DRED parse/process/recovery, cached recovery lifecycle/cursor bookkeeping, parser availability, the exercised encoder carried-payload matrix plus SILK wideband 20 ms primary-budget seam, internal converter/payload/basic-analysis seams, and 48 kHz runtime bootstrap checks
- the broader decoder explicit/live numerical parity matrix and real-model RDOVAE/PitchDNN/conceal-analysis oracles remain in the separate `test-unsupported-controls-parity-experimental` lane outside the production gate until their Linux matrix is green
...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the narrow SILK wideband 20 ms carried-payload/primary-budget seam pinned in the required DRED parity gate
- the carried SILK/Hybrid encoder packet-parity matrix now blocks production for the exercised `20/40/60 ms` mono and `20 ms` stereo seams; remaining encoder work is broadening rate, packet-shape, stereo, and multistream coverage before any wider support claim
...
- finish the same no-link compile-footprint split for top-level decoder and multistream DRED internals without weakening the existing dormant runtime contract or shared decoder tests
