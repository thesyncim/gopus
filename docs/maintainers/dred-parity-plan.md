...
- multistream DRED/PLC neural concealment: `setDNNBlob(...)` on the
  multistream decoder is capability-only by design (no per-stream neural
  consumer yet). Single-stream DRED runtime now accepts 16 kHz / 48 kHz one- or
  two-channel decoders; stereo uses the libopus-shaped mono-downmix-in /
  mono-duplicate-out neural path and remains quarantine-scoped until the broader
  stereo parity matrix is green.
...
- RESOLVED: Hybrid DRED latent generation now mirrors libopus's `nb_frames`
  ...
  matches libopus. The opt-in
  `TestEncoderCarriedDREDPayloadMatchesLibopusHybridFullband40msStereo`
  diagnostic still skips on packet length (gopus 160 B vs libopus 147 B; first
  primary frame 82 B vs 69 B), which is the skipped Hybrid/SILK stereo
  primary-frame byte-alignment lane rather than a DRED payload split gap.
