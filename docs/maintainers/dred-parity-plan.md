...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms mono and 20 ms stereo carried-payload/primary-frame seams, 20 ms primary-budget seam, and Hybrid fullband 20 ms payload/frame-index/SILK-VBR-budget seam pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the remaining encoder packet-shape gap is Hybrid primary-frame byte exactness; the current Hybrid smoke covers packet length, emission frame index, offset, and DRED payload only, while broader Hybrid packet shape still needs a separate libopus-backed fix before support docs can claim it
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-extension primary-frame seams, the Hybrid fullband 20 ms payload/frame-index carried-extension seam, the Hybrid SILK VBR cap used under that carried-extension budget, and primary DRED budget reservation on the covered SILK case, while Hybrid primary-frame byte exactness, broader packet-shape coverage, and final support-surface decisions remain open
