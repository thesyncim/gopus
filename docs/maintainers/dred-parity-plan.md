...
- encoder optional-extra ownership is now tighter too: DRED controls/runtime sit behind a lazy sidecar, `SetDNNBlob(...)` only binds model families, the runtime materializes only when model+duration+eligible mono or stereo-downmix use actually arms it, and disabling/resetting DRED drops that sidecar back to a dormant state
