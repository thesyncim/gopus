...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms carried-payload seams plus 20 ms primary-budget seam and Hybrid fullband 20 ms payload-only seam pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the remaining strict Hybrid packet-length/primary-shape and stereo encoder packet-parity matrix stays in the experimental lane through `make test-unsupported-controls-parity-experimental`; remaining encoder work is closing that currently exercised Hybrid `20/40 ms` strict mono and SILK/Hybrid `20 ms` stereo matrix before it can block production
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-extension seams, the Hybrid fullband 20 ms payload-only carried-extension seam, and primary DRED budget reservation on the covered SILK case, while strict Hybrid packet-length/primary-shape, broader packet-shape coverage, and final support-surface decisions remain open
