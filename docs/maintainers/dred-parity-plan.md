...
- Hybrid primary-frame byte exactness, broader stereo/multistream decoder coverage, and support-surface graduation remain open; Hybrid packet-length/payload smoke remains outside supported-tag claims
...
- the remaining encoder packet-shape gap is Hybrid primary-frame byte exactness; the current Hybrid smoke covers packet length, offset, and DRED payload only, while broader Hybrid packet shape still needs a separate libopus-backed fix before support docs can claim it
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-extension primary-frame seams, the Hybrid fullband 20 ms payload-only carried-extension seam, and primary DRED budget reservation on the covered SILK case, while Hybrid primary-frame byte exactness, broader packet-shape coverage, and final support-surface decisions remain open
