...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms carried-payload seams plus 20 ms primary-budget seam pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the remaining Hybrid/stereo encoder packet-parity matrix stays in the experimental lane through `make test-unsupported-controls-parity-experimental`; remaining encoder work is closing that currently exercised Hybrid `20/40 ms` mono and SILK/Hybrid `20 ms` stereo matrix before it can block production
