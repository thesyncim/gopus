...
- these decoder parity claims are seam-specific and libopus-backed, but the decoder audio numerical matrix currently remains experimental because Linux still exposes CELT/Hybrid cached-live and explicit DRED drift; the required gate is limited to selected green non-decoder-audio parity seams, while broader decoder, stereo, and multistream packet coverage remains separate work unless explicitly covered by green parity tests
...
- the required unsupported-controls gate is now intentionally limited to the green quarantine/core and selected non-decoder-audio parity surface: API exposure, standalone DRED parse/process/recovery, cached recovery lifecycle/cursor bookkeeping, parser availability, the narrow SILK 20 ms carried-payload/primary-budget seam, and the internal converter/payload/basic-analysis seams that are already libopus-backed and stable
- the broader decoder explicit/live numerical parity matrix, wider encoder carried-payload matrix, and real-model RDOVAE/PitchDNN/conceal-analysis oracles now run in the separate `test-unsupported-controls-parity-experimental` lane outside the production gate, while support claims remain limited to the green seam-specific libopus-backed tests
...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the narrow SILK 20 ms carried-payload/primary-budget seam pinned in the required gate and the wider carried-payload matrix kept experimental
- the carried SILK/Hybrid encoder packet-parity matrix now stays in the DRED parity lane, but only the Linux-green SILK 20 ms mono seam blocks production; remaining encoder work is stabilizing the wider `20/40/60 ms` mono and `20 ms` stereo matrix before deciding what graduates from quarantine
