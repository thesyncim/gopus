...
- the required supported DRED tag gate now carries standalone DRED wrapper lifecycle/no-allocation, libopus parse/decode/process metadata coverage, real-packet standalone process state/feature parity, standalone recovery scheduling parity, decoder cached recovery bookkeeping parity, the SILK wideband 20/40/60 ms mono and 20 ms stereo encoder carried-payload/primary-frame proofs plus the 20 ms primary-budget proof, and the Hybrid fullband 20 ms payload-only proof, while the unsupported-controls gate covers the green quarantine/core and selected non-decoder-audio parity surface: API exposure, parser availability, those same encoder seams, internal converter/payload/basic-analysis seams, the real-model PitchDNN and RDOVAE encoder oracles, the conceal-analysis oracle, and 48 kHz runtime bootstrap checks
- Hybrid packet-length parity, Hybrid primary-frame byte exactness, and the broader decoder explicit/live numerical parity matrix remain outside the production gate until their Linux matrix is green
...
- encoder optional-extra ownership is now tighter too: DRED controls/runtime sit behind a lazy sidecar, `SetDNNBlob(...)` only binds model families, the runtime materializes only when model+duration+eligible mono or stereo-downmix use actually arms it, disabling/resetting DRED drops that sidecar back to a dormant state, tagged builds now pin `Encode` with DRED-capable models but no DRED duration as zero-allocation and runtime-dormant, and the internal encoder DRED runtime files are build-tag split so default `./encoder` builds use no-op stubs instead of importing `internal/dred` or `internal/lpcnetplc`
...
- encoder carried DRED now reserves the libopus-shaped DRED bitrate from the primary single-frame SILK/short-Hybrid budget, with the SILK wideband 20/40/60 ms mono and 20 ms stereo carried-payload/primary-frame seams, 20 ms primary-budget seam, and Hybrid fullband 20 ms payload-only seam pinned in the supported DRED tag gate and mirrored in the required unsupported-controls DRED parity gate
- the remaining encoder packet-shape gaps are Hybrid packet-length parity and Hybrid primary-frame byte exactness; the current supported Hybrid seam covers offset and DRED payload only, while broader Hybrid packet shape still needs a separate libopus-backed fix before support docs can claim it
...
- the current pure-Go encoder seam now covers retained `8/12/16/24/48 kHz` mono and stereo-downmix conversion into the 16 kHz latent/state path, the libopus `dred_encode_silk_frame()` payload-coding seam, bounded single-frame carried extensions, exercised SILK carried-extension primary-frame seams, the Hybrid fullband 20 ms payload-only carried-extension seam, and primary DRED budget reservation on the covered SILK case, while Hybrid packet-length parity, Hybrid primary-frame byte exactness, broader packet-shape coverage, and final support-surface decisions remain open
