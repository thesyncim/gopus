...
test-dred-tag: ensure-libopus
	...
	run_json env GOPUS_TEST_TIER=parity GOPUS_STRICT_LIBOPUS_REF=1 $(GO_WORK_ENV) $(GO) test -tags gopus_dred ./multistream -run 'Test(DREDBuildTagExposesEncoderControlsOnly|DecoderPublicSetDNNBlobArmsDREDDecoderWhenBlobContainsModel|DecoderDecodeNilConsumesMultistreamDREDNeuralStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKStereoStream)' -count=1; \
	if grep -q '"Action":"skip"' "$$json_out"; then \
...
test-unsupported-controls-tag: ensure-libopus
	...
	run_json env GOPUS_TEST_TIER=parity GOPUS_STRICT_LIBOPUS_REF=1 $(GO_WORK_ENV) $(GO) test -tags gopus_unsupported_controls ./multistream -run 'Test(DecoderPublicSetDNNBlobArmsDREDDecoderWhenBlobContainsModel|DecoderDecodeNilConsumesMultistreamDREDNeuralStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKStereoStream)' -count=1; \
