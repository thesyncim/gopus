...
# Non-blocking decoder DRED parity sweep. This intentionally stays outside the
# required quarantine gate until the explicit/live numerical matrix is green.
test-unsupported-controls-parity-experimental: ensure-libopus
	...
	$(GO_WORK_ENV) $(GO) test -tags gopus_unsupported_controls ./internal/lpcnetplc -run 'Test(LPCNetSingleFrameFeaturesFloatMatchesLibopusColdStart|LPCNetSingleFrameFeaturesFloatMatchesLibopusStatefulSequence|PitchDNNMatchesLibopusOnRealModel|BurgCepstralAnalysisMatchesLibopus|ConcealFrameFloatWithAnalysisMatchesLibopus)' -count=1
	$(GO_WORK_ENV) $(GO) test -tags gopus_unsupported_controls . -run 'Test(EncoderCarriedDREDPayloadMatchesLibopusSilkWideband20ms|EncoderCarriedDREDPrimaryBudgetMatchesLibopusSilkWideband20ms|EncoderCarriedDREDPayloadMatchesLibopusHybridFullband20ms|EncoderCarriedDREDPayloadMatchesLibopusSilkWideband40ms|EncoderCarriedDREDPayloadMatchesLibopusHybridFullband40ms|EncoderCarriedDREDPayloadMatchesLibopusSilkWideband60ms|EncoderCarriedDREDPayloadMatchesLibopusSilkWideband20msStereo|EncoderCarriedDREDPayloadMatchesLibopusHybridFullband20msStereo)' -count=1
