...
test-dred-tag: ensure-libopus
	...
	run_json env GOPUS_TEST_TIER=parity GOPUS_STRICT_LIBOPUS_REF=1 $(GO_WORK_ENV) $(GO) test -tags gopus_dred ./multistream -run 'Test(DREDBuildTagExposesEncoderControlsOnly|DecoderPublicSetDNNBlobArmsDREDDecoderWhenBlobContainsModel|DecoderDecodeNilConsumesMultistreamDREDNeuralStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTSecondCoupledStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridSecondCoupledStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKSecondCoupledStream)' -count=1; \
	if grep -q '"Action":"skip"' "$$json_out"; then \
...
test-qext-parity: ensure-libopus-qext
	...
	run_json env GOPUS_TEST_TIER=parity GOPUS_STRICT_LIBOPUS_REF=1 $(GO_WORK_ENV) $(GO) test -tags gopus_qext . -run 'Test(OptionalExtensionDocsContract|SupportsOptionalExtension|QEXTBuildPublicAPIContract|QEXTBuildTagExposesSupportedTopLevelControls|Encoder_OptionalExtensionControls|MultistreamEncoder_OptionalExtensionControls|DecodeGopusQEXTPacketMatchesLibopus|DecodeLibopusQEXTPacketMatchesLibopus|DecodeLibopusQEXTPacketFinalRangeMatchesLibopus|DecodeLibopusQEXTPacketIgnoreExtensionsMatchesInactiveCELT|DecodeLibopusQEXTIgnoreExtensionsToggleSequenceMatchesExplicitPayloads|DecodeLibopusQEXTPacketCELTFloat32FastPathMatchesFloat64|DecodeLibopusQEXTPacketWrapperMatchesDirectCELT|DecodeHybridLibopusQEXTPacketMatchesLibopus|DecodeStereoLibopusQEXTPacketToMonoMatchesLibopus|DecodeLibopusRestrictedCELTPacketMatchesLibopus|DecodeLibopusQEXTChannelTransitionSequenceMatchesLibopus)|ExampleSupportsOptionalExtension' -count=1; \
...
test-unsupported-controls-tag: ensure-libopus
	...
	run_json env GOPUS_TEST_TIER=parity GOPUS_STRICT_LIBOPUS_REF=1 $(GO_WORK_ENV) $(GO) test -tags gopus_unsupported_controls ./multistream -run 'Test(DecoderPublicSetDNNBlobArmsDREDDecoderWhenBlobContainsModel|DecoderDecodeNilConsumesMultistreamDREDNeuralStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralCELTSecondCoupledStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralHybridSecondCoupledStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKStereoStream|DecoderDecodeNilConsumesMultistreamDREDNeuralSILKSecondCoupledStream)' -count=1; \
