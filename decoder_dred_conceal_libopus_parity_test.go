...
// Ordinary cached Decode(nil) follows libopus FRAME_PLC_NEURAL. The explicit
// DRED API follows FRAME_DRED, so this scaffold stays disabled in favor of the
// separate live-sequence and explicit decode tests.
func disabledDecoderFirstLossNeuralConcealmentMatchesExplicitDREDOracle(t *testing.T) {
...
func disabledDecoderSecondLossNeuralConcealmentMatchesExplicitDREDOracle(t *testing.T) {
	dec, pcm, packetInfo, n := prepareDecoderForNeuralConcealmentParity(t)
