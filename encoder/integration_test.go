...
func TestEncoderAllModes(t *testing.T) {
	for _, tc := range testConfigs {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify bandwidth matches (convert types.Bandwidth to types.Bandwidth for comparison)
			if toc.Bandwidth != types.Bandwidth(tc.bandwidth) {
...
// modeToGopusIntegration converts encoder.Mode to types.Mode
func modeToGopusIntegration(m encoder.Mode) types.Mode {
	switch m {
	...
	default:
		return types.ModeSILK // Auto defaults to SILK
	}
}
