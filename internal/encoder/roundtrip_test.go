		...
		for _, fs := range frameSizes {
			...
			t.Run(testName, func(t *testing.T) {
				...
				for i := range numFrames {
					...
					packet, err := encodeTest(enc, framePCM, fs.samples)
					if err != nil {
...
func TestRoundTripHybridFullband(t *testing.T) {
	...
	for i := range numFrames {
		...
		packet, err := encodeTest(enc, framePCM, frameSize)
		if err != nil {
			t.Fatalf("Encode failed: %v", err)
		}
...
func TestRoundTripAllFrameSizes(t *testing.T) {
	...
	for _, size := range celtSizes {
		t.Run("CELT_"+frameSizeToString(size), func(t *testing.T) {
			...
			packet, err := encodeTest(enc, pcm, size)
			if err != nil {
				t.Fatalf("Encode failed: %v", err)
			}
...
func TestModeTransitions(t *testing.T) {
	...
	for _, tr := range transitions {
		t.Run(tr.name, func(t *testing.T) {
			...
			packet1, err := encodeTest(enc, pcm1, frameSize)
			if err != nil {
			...
			packet2, err := encodeTest(enc, pcm2, frameSize)
			if err != nil {
...
func TestDTXPackets(t *testing.T) {
	...
	packet, err := encodeTest(enc, speech, frameSize)
	if err != nil {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := range numFrames {
				...
				packet, err := encodeTest(enc, framePCM, tc.frameSize)
				if err != nil {
...
func TestHighLevelAPIRoundTrip(t *testing.T) {
	...
	for _, app := range applications {
		t.Run(app.name, func(t *testing.T) {
			enc, err := gopus.NewEncoder(gopus.EncoderConfig{SampleRate: 48000, Channels: 2, Application: app.app})
			if err != nil {
