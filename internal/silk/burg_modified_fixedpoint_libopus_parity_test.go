...
var (
	libopusSILKFixedBurgOnce sync.Once
	libopusSILKFixedBurgBin  string
	libopusSILKFixedBurgErr  error
)

// buildLibopusSILKFixedBurgHelper ensures the FIXED_POINT libopus reference
// exists, then compiles tools/csrc/libopus_silk_fixed_burg_info.c against it.
func buildLibopusSILKFixedBurgHelper() (string, error) {
	libopusSILKFixedBurgOnce.Do(func() {
		...
		if _, err := os.Stat(staticLib); err != nil {
			...
			if out, berr := cmd.CombinedOutput(); berr != nil {
				libopusSILKFixedBurgErr = fmt.Errorf("ensure fixed libopus: %w (%s)", berr, out)
				return
			}
		}
		if _, err := os.Stat(staticLib); err != nil {
			libopusSILKFixedBurgErr = fmt.Errorf("fixed libopus static lib missing: %w", err)
			return
		}

		cc, err := libopustooling.FindCCompiler()
		if err != nil {
			libopusSILKFixedBurgErr = err
			return
		}

		src := filepath.Join(repoRoot, "tools", "csrc", "libopus_silk_fixed_burg_info.c")
		outDir := filepath.Join(os.TempDir(), "gopus_libopus_test_helpers")
		if err := os.MkdirAll(outDir, 0o755); err != nil {
			libopusSILKFixedBurgErr = err
			return
		}
		out := filepath.Join(outDir, fmt.Sprintf("gopus_silk_fixed_burg_%s_%s", runtime.GOOS, runtime.GOARCH))
		...
		if combined, cerr := cmd.CombinedOutput(); cerr != nil {
			libopusSILKFixedBurgErr = fmt.Errorf("build silk fixed burg helper: %w (%s)", cerr, combined)
			return
		}
		libopusSILKFixedBurgBin = out
	})
	return libopusSILKFixedBurgBin, libopusSILKFixedBurgErr
}
...
func probeLibopusSILKFixedBurg(cases []silkFixedBurgCase) ([]silkFixedBurgResult, error) {
	binPath, err := buildLibopusSILKFixedBurgHelper()
	if err != nil {
		return nil, err
	}
