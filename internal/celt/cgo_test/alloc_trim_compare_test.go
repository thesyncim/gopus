...
func TestAllocTrimCompareWithEnc(t *testing.T) {
	...
	// Enable AllocTrim debug
	celt.DebugAllocTrim = true
	defer func() { celt.DebugAllocTrim = false }()
	...
	// Compare raw bytes
	t.Log("=== Raw packet byte comparison ===")
	t.Logf("gopus packet length: %d", len(goPacket))
	t.Logf("libopus packet length: %d (including TOC)", len(libPacket))
	minLen := len(goPacket)
	if len(libPacket)-1 < minLen {
		minLen = len(libPacket) - 1
	}
	if minLen > 20 {
		minLen = 20
	}
	t.Log("First 20 bytes:")
	t.Logf("  gopus:   %x", goPacket[:minLen])
	t.Logf("  libopus: %x (without TOC)", libPacket[1:1+minLen])
...
// TestAllocTrimSpectralTilt compares spectral tilt computation between gopus and libopus logic
