...
// mdctFMA32 computes a*b+c as float32(math.FMA(float64(a), float64(b),
// float64(c))). The float64 FMA rounds once, but the result is rounded again to
// float32, and when the exact sum needs more than 53 bits the two roundings can
