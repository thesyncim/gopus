...
// encoderVariantMinGapFloorPureGoOverrideQ loosens the gapQ floor for the pure-Go
// build (-tags nosimd, gopusBuildIsSIMD==false) on the Hybrid-FB knife-edge cases
// where libopus is itself arch-unstable. These cases are NOT a gopus defect: the
// variants fixture is generated by the SIMD/default libopus, but the pure-Go
// gopus float backend reproduces the SCALAR/portable libopus numerics (it
// byte-matches the arm64/scalar libopus on these signals — gopus goQ is FLAT
// across arm64-nosimd and amd64-nosimd). On these AM-multisine / chirp Hybrid-FB
// signals the amd64 SIMD libopus and the scalar libopus diverge in their CELT
// band-coding by ~4–9 Q, so the SIMD-fixture libQ (≈31.6 / 9.4 / 40.1) sits that
// far above the scalar-tracking gopus goQ (≈22.2 / 5.5 / 36.3). Matching the SIMD
// fixture would require replicating amd64-libopus's non-portable AVX-FMA
// reduction order, which would break the exact arm64/scalar match and is not a
// real fix. The amd64 asm/SIMD gopus build matches the SIMD fixture here (gapQ≈0)
// and is held to the tight amd64 floor. Floors below are the measured pure-Go
// gapQ plus headroom; everything else keeps the tight per-arch floor, so a real
// gopus quality regression still fails. See
// project_amd64_encoder_precision_regression.md and project_arm64_celt_1ulp_drift.md.
var encoderVariantMinGapFloorPureGoOverrideQ = map[string]float64{
	encoderVariantCaseKey("HYBRID-FB-20ms-stereo-96k", "am_multisine_v1"): -10.0,
	encoderVariantCaseKey("HYBRID-FB-10ms-mono-64k", "am_multisine_v1"):   -4.5,
	encoderVariantCaseKey("HYBRID-FB-10ms-mono-64k", "chirp_sweep_v1"):    -4.5,
}
...
func encoderVariantThresholdForArch(c encoderComplianceVariantsFixtureCase, goarch string) encoderVariantParityThreshold {
	...
	pureGoFloor, hasPureGoFloor := encoderVariantMinGapFloorPureGoOverrideQ[key]
	amd64Floor, hasAMD64Floor := encoderVariantMinGapFloorAMD64OverrideQ[key]
	if goarch == "amd64" {
		switch {
		case !gopusBuildIsSIMD && hasPureGoFloor:
			// Pure-Go build vs the SIMD variants fixture on a Hybrid-FB knife-edge
			// case where libopus is itself arch-unstable; gopus tracks the scalar
			// libopus. See encoderVariantMinGapFloorPureGoOverrideQ.
			out.minGapQ = pureGoFloor
		case hasAMD64Floor:
			out.minGapQ = amd64Floor
		case out.minGapQ < amd64EncoderVariantGapFloorQ:
...
func encodeGopusForVariantsCase(c encoderComplianceVariantsFixtureCase, signal []float32) ([][]byte, error) {
	...
	// CELT rows are generated with `opus_demo -e restricted-celt`, which maps to
	// CELT-only with zero top-level delay compensation in libopus.
	encMode := mode
	...
	// Match fixture generation (opus_demo app/profile/complexity + bitrate controls only):
