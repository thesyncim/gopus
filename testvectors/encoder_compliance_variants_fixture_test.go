...
// build (-tags purego, gopusBuildIsAsm==false) on the Hybrid-FB knife-edge cases
...
// across arm64-purego and amd64-purego). On these AM-multisine / chirp Hybrid-FB
...
func encoderVariantThresholdForArch(c encoderComplianceVariantsFixtureCase, goarch string) encoderVariantParityThreshold {
	...
	if goarch == "amd64" {
		switch {
		case !gopusBuildIsAsm && hasPureGoFloor:
