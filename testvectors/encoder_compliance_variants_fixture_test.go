...
// Quality measurement uses the matched live libopus build selected for the active lane.
const (
...
func encoderVariantThresholdForArch(c encoderComplianceVariantsFixtureCase, goarch string) encoderVariantParityThreshold {
	...
	// Apply the architecture floor against the matched live reference.
	if goarch == "amd64" {
		if out.minGapQ < amd64EncoderVariantGapFloorQ {
...
func measureEncoderVariantParityCase(t *testing.T, c encoderComplianceVariantsFixtureCase) (encoderVariantParityMeasurement, error) {
	...
	if err != nil {
		return encoderVariantParityMeasurement{}, fmt.Errorf("map variant reference settings: %w", err)
	}
	...
	libRef, err := runPairedLibopusQualityReference(settings, signal)
	if err != nil {
