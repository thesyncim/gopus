...
func TestTmpSILKComplexitySweep(t *testing.T) {
    ...
    for _, cx := range []int{0, 2, 4, 6, 8, 10} {
        ...
        ogg := &bytes.Buffer{}
        if err := writeOggOpusEncoder(ogg, packets, channels, sampleRate, frameSize); err != nil {
            t.Fatalf("cx=%d write ogg: %v", cx, err)
        }
        decoded, err := decodeWithOpusdec(ogg.Bytes())
        if err != nil {
            if os.IsPermission(err) || err.Error() == "opusdec blocked by macOS provenance" {
                t.Skipf("cx=%d opusdec blocked by macOS provenance", cx)
            }
