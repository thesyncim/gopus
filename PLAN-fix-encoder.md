...
- [ ] **Fix `alloc.go` lines 693-699**: Encoder only encodes skip bit when NOT skipping, but decoder always reads it
- [ ] Match libopus behavior: `clt_compute_allocation()` in `rate.c` encodes skip bit unconditionally in the loop
- [ ] Reference: `tmp_check/opus-1.6.1/celt/rate.c` lines 500-530
- [ ] Add test: verify encoder/decoder produce same bit consumption per band

### Issue 8: Skip Bit Semantics Reversed
- [ ] **Fix skip bit polarity in `alloc.go`**: Encoder encodes `1` for "keep", decoder reads `1` as "skip"
- [ ] Align semantics with libopus: `1` = keep band (continue), `0` = skip remaining bands
...
- [ ] **Fix `energy_encode.go` lines 86-95**: Energy is mean-relative but then eMeans added back during normalization
- [ ] Solution: Either keep energies absolute everywhere, or ensure consistent subtraction/addition
...
- [ ] **Fix `energy_encode.go` line 259**: `e.prevEnergy` modified during `EncodeCoarseEnergy()` but `prev1LogE` captured before
- [ ] Solution: Don't modify state during encoding; update state after encoding completes
- [ ] Move `e.prevEnergy[c*MaxBands+band] = quantizedEnergy` to happen AFTER encoding loop
...
- [ ] **Fix `bands_encode.go` lines 59-62**: Ensure normalization uses consistent energy domain
- [ ] Verify gain computation matches decoder's `denormalize_bands()` exactly
...
- [ ] **Fix `encode_frame.go` line 233**: Remove duplicate `e.frameCount++`
- [ ] Verify: intra mode used only for frame 0, inter mode for all subsequent frames
- [ ] Add test: encode 5 frames, verify frame 0 is intra, frames 1-4 are inter

### Issue 7: Overlap Buffer Not Persisted
- [ ] **Fix `encode_frame.go` lines 72-108**: Store updated overlap buffer back to encoder state
- [ ] Add `e.overlapBuffer` persistent field to encoder struct
- [ ] Update after MDCT: `copy(e.overlapBuffer, updatedHistory)`
...
- [ ] **Fix `alloc.go` lines 716-743**: Align intensity/dualStereo encoding with decoder expectations
- [ ] Ensure `intensityRsv` and `dualStereoRsv` computed same as decoder
- [ ] Reference: `tmp_check/opus-1.6.1/celt/rate.c` `clt_compute_allocation()` lines 580-620

### Issue 5: Missing Stereo Parameter Encoding
- [ ] **Add `EncodeStereoParams()` call to `encode_frame.go`**:
  - After coarse energy, before allocation
  - Encode intensity band index (if stereo)
  - Encode dual_stereo flag
- [ ] Match decoder's `decodeStereoParams()` exactly
...
- [ ] **Fix `encode_frame.go` `computeTargetBits()`**: Provide sensible defaults
- [ ] Default bitrate: 64kbps mono, 128kbps stereo
- [ ] Validate bitrate range: 6kbps - 510kbps per RFC 6716
