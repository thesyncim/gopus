...
func TestNSQDelDecMatchesLibopusCapturedOpusState(t *testing.T) {
	const (
		sampleRate = 48000
		channels   = 1
		bitrate    = 32000
	...
	signal := generateNLSFTraceSignal(frames*frameSize, sampleRate)
	for frame := 0; frame < frames; frame++ {
		...
		libPulses, libXQ, libSeed, _, _, _, libFinal := cgowrap.SilkNSQDelDecCaptureWithStateFinal(
		...
		if idx := firstInt8Mismatch(goPulses, libPulses); idx >= 0 {
			t.Fatalf("frame %d: pulse[%d] mismatch go=%d lib=%d", frame, idx, goPulses[idx], libPulses[idx])
		}
		if idx := firstInt16Mismatch(goXQ, libXQ); idx >= 0 {
			t.Fatalf("frame %d: xq[%d] mismatch go=%d lib=%d", frame, idx, goXQ[idx], libXQ[idx])
		}
		if goSeed != libSeed {
			t.Fatalf("frame %d: seed mismatch go=%d lib=%d", frame, goSeed, libSeed)
		}
		if idx := firstInt16Mismatch(nsq.xq[:], libFinal.XQ); idx >= 0 {
			t.Fatalf("frame %d: final xq state mismatch at %d", frame, idx)
		}
	}
}

func firstInt8Mismatch(a, b []int8) int {
	n := len(a)
	if len(b) < n {
		n = len(b)
	}
