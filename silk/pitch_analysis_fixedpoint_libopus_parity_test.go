...
var (
	libopusSILKFixedPitchEnergyOnce sync.Once
	libopusSILKFixedPitchEnergyBin  string
	libopusSILKFixedPitchEnergyErr  error
)

// buildLibopusSILKFixedPitchEnergyHelper ensures the FIXED_POINT libopus
// reference exists, then compiles
// tools/csrc/libopus_silk_fixed_pitch_analysis_info.c against it.
func buildLibopusSILKFixedPitchEnergyHelper() (string, error) {
	libopusSILKFixedPitchEnergyOnce.Do(func() {
		...
		if _, err := os.Stat(staticLib); err != nil {
			...
			if out, berr := cmd.CombinedOutput(); berr != nil {
				libopusSILKFixedPitchEnergyErr = fmt.Errorf("ensure fixed libopus: %w (%s)", berr, out)
				return
			}
		}
		if _, err := os.Stat(staticLib); err != nil {
			libopusSILKFixedPitchEnergyErr = fmt.Errorf("fixed libopus static lib missing: %w", err)
			return
		}

		cc, err := libopustooling.FindCCompiler()
		if err != nil {
			libopusSILKFixedPitchEnergyErr = err
			return
		}

		src := filepath.Join(repoRoot, "tools", "csrc", "libopus_silk_fixed_pitch_analysis_info.c")
		outDir := filepath.Join(os.TempDir(), "gopus_libopus_test_helpers")
		if err := os.MkdirAll(outDir, 0o755); err != nil {
			libopusSILKFixedPitchEnergyErr = err
			return
		}
		out := filepath.Join(outDir, fmt.Sprintf("gopus_silk_fixed_pitch_energy_%s_%s", runtime.GOOS, runtime.GOARCH))
		...
		if combined, cerr := cmd.CombinedOutput(); cerr != nil {
			libopusSILKFixedPitchEnergyErr = fmt.Errorf("build silk fixed pitch energy helper: %w (%s)", cerr, combined)
			return
		}
		libopusSILKFixedPitchEnergyBin = out
	})
	return libopusSILKFixedPitchEnergyBin, libopusSILKFixedPitchEnergyErr
}
...
func probeLibopusSILKFixedPitchEnergy(cases []silkFixedPitchEnergyCase) ([][]int32, error) {
	binPath, err := buildLibopusSILKFixedPitchEnergyHelper()
	if err != nil {
		return nil, err
	}
