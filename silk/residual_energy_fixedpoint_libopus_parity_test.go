...
var (
	libopusSILKFixedResEnergyOnce sync.Once
	libopusSILKFixedResEnergyBin  string
	libopusSILKFixedResEnergyErr  error
)

// buildLibopusSILKFixedResEnergyHelper ensures the FIXED_POINT libopus
// reference exists, then compiles
// tools/csrc/libopus_silk_fixed_residual_energy_info.c against it.
func buildLibopusSILKFixedResEnergyHelper() (string, error) {
	libopusSILKFixedResEnergyOnce.Do(func() {
		...
		if _, err := os.Stat(staticLib); err != nil {
			...
			if out, berr := cmd.CombinedOutput(); berr != nil {
				libopusSILKFixedResEnergyErr = fmt.Errorf("ensure fixed libopus: %w (%s)", berr, out)
				return
			}
		}
		if _, err := os.Stat(staticLib); err != nil {
			libopusSILKFixedResEnergyErr = fmt.Errorf("fixed libopus static lib missing: %w", err)
			return
		}

		cc, err := libopustooling.FindCCompiler()
		if err != nil {
			libopusSILKFixedResEnergyErr = err
			return
		}

		src := filepath.Join(repoRoot, "tools", "csrc", "libopus_silk_fixed_residual_energy_info.c")
		outDir := filepath.Join(os.TempDir(), "gopus_libopus_test_helpers")
		if err := os.MkdirAll(outDir, 0o755); err != nil {
			libopusSILKFixedResEnergyErr = err
			return
		}
		out := filepath.Join(outDir, fmt.Sprintf("gopus_silk_fixed_resenergy_%s_%s", runtime.GOOS, runtime.GOARCH))
		...
		if combined, cerr := cmd.CombinedOutput(); cerr != nil {
			libopusSILKFixedResEnergyErr = fmt.Errorf("build silk fixed residual energy helper: %w (%s)", cerr, combined)
			return
		}
		libopusSILKFixedResEnergyBin = out
	})
	return libopusSILKFixedResEnergyBin, libopusSILKFixedResEnergyErr
}
...
func probeLibopusSILKFixedResEnergy(cases []silkFixedResEnergyCase) ([]silkFixedResEnergyResult, error) {
	binPath, err := buildLibopusSILKFixedResEnergyHelper()
	if err != nil {
		return nil, err
	}
