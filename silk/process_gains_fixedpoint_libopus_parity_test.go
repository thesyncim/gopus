...
var (
	libopusSILKFixedProcessGainsOnce sync.Once
	libopusSILKFixedProcessGainsBin  string
	libopusSILKFixedProcessGainsErr  error
)

// buildLibopusSILKFixedProcessGainsHelper ensures the FIXED_POINT libopus
// reference exists, then compiles
// tools/csrc/libopus_silk_fixed_process_gains_info.c against it.
func buildLibopusSILKFixedProcessGainsHelper() (string, error) {
	libopusSILKFixedProcessGainsOnce.Do(func() {
		...
		if _, err := os.Stat(staticLib); err != nil {
			...
			if out, berr := cmd.CombinedOutput(); berr != nil {
				libopusSILKFixedProcessGainsErr = fmt.Errorf("ensure fixed libopus: %w (%s)", berr, out)
				return
			}
		}
		if _, err := os.Stat(staticLib); err != nil {
			libopusSILKFixedProcessGainsErr = fmt.Errorf("fixed libopus static lib missing: %w", err)
			return
		}

		cc, err := libopustooling.FindCCompiler()
		if err != nil {
			libopusSILKFixedProcessGainsErr = err
			return
		}

		src := filepath.Join(repoRoot, "tools", "csrc", "libopus_silk_fixed_process_gains_info.c")
		outDir := filepath.Join(os.TempDir(), "gopus_libopus_test_helpers")
		if err := os.MkdirAll(outDir, 0o755); err != nil {
			libopusSILKFixedProcessGainsErr = err
			return
		}
		out := filepath.Join(outDir, fmt.Sprintf("gopus_silk_fixed_process_gains_%s_%s", runtime.GOOS, runtime.GOARCH))
		...
		if combined, cerr := cmd.CombinedOutput(); cerr != nil {
			libopusSILKFixedProcessGainsErr = fmt.Errorf("build silk fixed process gains helper: %w (%s)", cerr, combined)
			return
		}
		libopusSILKFixedProcessGainsBin = out
	})
	return libopusSILKFixedProcessGainsBin, libopusSILKFixedProcessGainsErr
}
...
func probeLibopusSILKFixedProcessGains(cases []silkFixedProcessGainsCase) ([]silkFixedProcessGainsResult, error) {
	binPath, err := buildLibopusSILKFixedProcessGainsHelper()
	if err != nil {
		return nil, err
	}
