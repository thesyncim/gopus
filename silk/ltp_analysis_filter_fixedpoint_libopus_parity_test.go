...
var (
	libopusSILKFixedLTPAnalysisOnce sync.Once
	libopusSILKFixedLTPAnalysisBin  string
	libopusSILKFixedLTPAnalysisErr  error
)

// buildLibopusSILKFixedLTPAnalysisHelper ensures the FIXED_POINT libopus
// reference exists, then compiles
// tools/csrc/libopus_silk_fixed_ltp_analysis_filter_info.c against it.
func buildLibopusSILKFixedLTPAnalysisHelper() (string, error) {
	libopusSILKFixedLTPAnalysisOnce.Do(func() {
		...
		if _, err := os.Stat(staticLib); err != nil {
			...
			if out, berr := cmd.CombinedOutput(); berr != nil {
				libopusSILKFixedLTPAnalysisErr = fmt.Errorf("ensure fixed libopus: %w (%s)", berr, out)
				return
			}
		}
		if _, err := os.Stat(staticLib); err != nil {
			libopusSILKFixedLTPAnalysisErr = fmt.Errorf("fixed libopus static lib missing: %w", err)
			return
		}

		cc, err := libopustooling.FindCCompiler()
		if err != nil {
			libopusSILKFixedLTPAnalysisErr = err
			return
		}

		src := filepath.Join(repoRoot, "tools", "csrc", "libopus_silk_fixed_ltp_analysis_filter_info.c")
		outDir := filepath.Join(os.TempDir(), "gopus_libopus_test_helpers")
		if err := os.MkdirAll(outDir, 0o755); err != nil {
			libopusSILKFixedLTPAnalysisErr = err
			return
		}
		out := filepath.Join(outDir, fmt.Sprintf("gopus_silk_fixed_ltp_analysis_%s_%s", runtime.GOOS, runtime.GOARCH))
		...
		if combined, cerr := cmd.CombinedOutput(); cerr != nil {
			libopusSILKFixedLTPAnalysisErr = fmt.Errorf("build silk fixed ltp analysis helper: %w (%s)", cerr, combined)
			return
		}
		libopusSILKFixedLTPAnalysisBin = out
	})
	return libopusSILKFixedLTPAnalysisBin, libopusSILKFixedLTPAnalysisErr
}
...
func probeLibopusSILKFixedLTPAnalysis(cases []silkFixedLTPAnalysisCase) ([]silkFixedLTPAnalysisResult, error) {
	binPath, err := buildLibopusSILKFixedLTPAnalysisHelper()
	if err != nil {
		return nil, err
	}
