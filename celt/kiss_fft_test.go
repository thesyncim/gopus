...
func TestKissIFFT64_MatchesIDFT(t *testing.T) {
	...
	for _, n := range sizes {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			// Compute using direct IDFT (which includes 1/n scaling)
			idftOut := directIDFT64(x)

			// Compare - KissIFFT does NOT include 1/n scaling, but directIDFT64 does,
			// so we need to scale the KissIFFT output by 1/n before comparing
			maxErr := 0.0
			scale := 1.0 / float64(n)
			for i := 0; i < n; i++ {
				err := cmplx.Abs(ifftOut[i]*complex(scale, 0) - idftOut[i])
				if err > maxErr {
					maxErr = err
				}
			}
...
func TestDftTo_UsesEfficientFFT(t *testing.T) {
	...
	for _, n := range sizes {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			// Allow slightly larger tolerance for larger FFT sizes (n=240 can have ~1.2e-9 error)
			if maxErr > 2e-9 {
				t.Errorf("Max error = %e, want < 2e-9", maxErr)
			}
		})
	}
}
