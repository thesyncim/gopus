...
void test_silk_nsq_del_dec_capture_with_state_out_state(
    int frame_length, int subfr_length, int nb_subfr, int ltp_mem_length,
    int pred_lpc_order, int shape_lpc_order, int warping_Q16, int nStates,
    int signalType, int quantOffsetType, int nlsfInterpCoef_Q2, int seed,
    const opus_int16 *x16,
    ...
    opus_int32 *out_delayed_gain_q10,
    int out_delayed_gain_len,
    opus_int16 *out_state_xq,
    int out_state_xq_len,
    opus_int32 *out_state_sltp_shp_q14,
    int out_state_sltp_shp_len,
    opus_int32 *out_state_sLPC_Q14,
    int out_state_sLPC_len,
    opus_int32 *out_state_sAR2_Q14,
    int out_state_sAR2_len,
    opus_int32 *out_state_lf_ar_q14,
    opus_int32 *out_state_diff_q14,
    int *out_state_lag_prev,
    int *out_state_sltp_buf_idx,
    int *out_state_sltp_shp_buf_idx,
    opus_int32 *out_state_rand_seed,
    opus_int32 *out_state_prev_gain_q16,
    int *out_state_rewhite_flag
) {
    silk_encoder_state enc;
    silk_nsq_state nsq;
    SideInfoIndices indices;
    ...
    if (out_state_xq && out_state_xq_len > 0) {
        n = out_state_xq_len < (int)(sizeof(nsq.xq)/sizeof(nsq.xq[0])) ? out_state_xq_len : (int)(sizeof(nsq.xq)/sizeof(nsq.xq[0]));
        memcpy(out_state_xq, nsq.xq, n * sizeof(opus_int16));
    }
    if (out_state_sltp_shp_q14 && out_state_sltp_shp_len > 0) {
        n = out_state_sltp_shp_len < (int)(sizeof(nsq.sLTP_shp_Q14)/sizeof(nsq.sLTP_shp_Q14[0])) ? out_state_sltp_shp_len : (int)(sizeof(nsq.sLTP_shp_Q14)/sizeof(nsq.sLTP_shp_Q14[0]));
        memcpy(out_state_sltp_shp_q14, nsq.sLTP_shp_Q14, n * sizeof(opus_int32));
    }
    if (out_state_sLPC_Q14 && out_state_sLPC_len > 0) {
        n = out_state_sLPC_len < (int)(sizeof(nsq.sLPC_Q14)/sizeof(nsq.sLPC_Q14[0])) ? out_state_sLPC_len : (int)(sizeof(nsq.sLPC_Q14)/sizeof(nsq.sLPC_Q14[0]));
        memcpy(out_state_sLPC_Q14, nsq.sLPC_Q14, n * sizeof(opus_int32));
    }
    if (out_state_sAR2_Q14 && out_state_sAR2_len > 0) {
        n = out_state_sAR2_len < (int)(sizeof(nsq.sAR2_Q14)/sizeof(nsq.sAR2_Q14[0])) ? out_state_sAR2_len : (int)(sizeof(nsq.sAR2_Q14)/sizeof(nsq.sAR2_Q14[0]));
        memcpy(out_state_sAR2_Q14, nsq.sAR2_Q14, n * sizeof(opus_int32));
    }
    if (out_state_lf_ar_q14) {
        *out_state_lf_ar_q14 = nsq.sLF_AR_shp_Q14;
    }
    if (out_state_diff_q14) {
        *out_state_diff_q14 = nsq.sDiff_shp_Q14;
    }
    if (out_state_lag_prev) {
        *out_state_lag_prev = nsq.lagPrev;
    }
    if (out_state_sltp_buf_idx) {
        *out_state_sltp_buf_idx = nsq.sLTP_buf_idx;
    }
    if (out_state_sltp_shp_buf_idx) {
        *out_state_sltp_shp_buf_idx = nsq.sLTP_shp_buf_idx;
    }
    if (out_state_rand_seed) {
        *out_state_rand_seed = nsq.rand_seed;
    }
    if (out_state_prev_gain_q16) {
        *out_state_prev_gain_q16 = nsq.prev_gain_Q16;
    }
    if (out_state_rewhite_flag) {
        *out_state_rewhite_flag = nsq.rewhite_flag;
    }
...
// SilkNSQDelDecCaptureWithStateFinal runs libopus NSQ_del_dec with a seeded
// NSQ state snapshot and returns both output vectors and final NSQ state.
func SilkNSQDelDecCaptureWithStateFinal(
	frameLength, subfrLength, nbSubfr, ltpMemLength int,
	predLPCOrder, shapeLPCOrder, warpingQ16, nStates int,
	signalType, quantOffsetType, nlsfInterpCoefQ2, seed int,
	x16 []int16,
	...
	statePrevGainQ16 int32,
	stateRewhiteFlag int,
) ([]int8, []int16, int, []int32, []int16, []int32, SilkNSQStateSnapshot) {
	empty := SilkNSQStateSnapshot{}
	if frameLength <= 0 || len(x16) < frameLength {
		return nil, nil, seed, nil, nil, nil, empty
	}
	if len(predCoefQ12) == 0 || len(ltpCoefQ14) == 0 || len(arShpQ13) == 0 ||
		len(harmShapeGainQ14) == 0 || len(tiltQ14) == 0 || len(lfShpQ14) == 0 ||
		len(gainsQ16) == 0 || len(pitchL) == 0 ||
		len(stateXQ) == 0 || len(stateSLTPShpQ14) == 0 || len(stateSLPCQ14) == 0 || len(stateSAR2Q14) == 0 {
		return nil, nil, seed, nil, nil, nil, empty
	}
	...
	stateXQOut := make([]int16, len(stateXQ))
	stateSLTPShpOut := make([]int32, len(stateSLTPShpQ14))
	stateSLPCOut := make([]int32, len(stateSLPCQ14))
	stateSAR2Out := make([]int32, len(stateSAR2Q14))
	...
	var cSeed C.int
	var outLFAR C.opus_int32
	var outDiff C.opus_int32
	var outLagPrev C.int
	var outSLTPBufIdx C.int
	var outSLTPShpBufIdx C.int
	var outRandSeed C.opus_int32
	var outPrevGain C.opus_int32
	var outRewhite C.int

	C.test_silk_nsq_del_dec_capture_with_state_out_state(
		C.int(frameLength), C.int(subfrLength), C.int(nbSubfr), C.int(ltpMemLength),
		C.int(predLPCOrder), C.int(shapeLPCOrder), C.int(warpingQ16), C.int(nStates),
		C.int(signalType), C.int(quantOffsetType), C.int(nlsfInterpCoefQ2), C.int(seed),
		(*C.opus_int16)(unsafe.Pointer(&x16[0])),
		...
		(*C.opus_int32)(unsafe.Pointer(&delayedGain[0])),
		C.int(len(delayedGain)),
		(*C.opus_int16)(unsafe.Pointer(&stateXQOut[0])),
		C.int(len(stateXQOut)),
		(*C.opus_int32)(unsafe.Pointer(&stateSLTPShpOut[0])),
		C.int(len(stateSLTPShpOut)),
		(*C.opus_int32)(unsafe.Pointer(&stateSLPCOut[0])),
		C.int(len(stateSLPCOut)),
		(*C.opus_int32)(unsafe.Pointer(&stateSAR2Out[0])),
		C.int(len(stateSAR2Out)),
		&outLFAR,
		&outDiff,
		&outLagPrev,
		&outSLTPBufIdx,
	...
	finalState := SilkNSQStateSnapshot{
		XQ:            stateXQOut,
		SLTPShpQ14:    stateSLTPShpOut,
		SLPCQ14:       stateSLPCOut,
		SAR2Q14:       stateSAR2Out,
		LFARQ14:       int32(outLFAR),
		DiffQ14:       int32(outDiff),
		LagPrev:       int(outLagPrev),
		SLTPBufIdx:    int(outSLTPBufIdx),
		SLTPShpBufIdx: int(outSLTPShpBufIdx),
		RandSeed:      int32(outRandSeed),
		PrevGainQ16:   int32(outPrevGain),
		RewhiteFlag:   int(outRewhite),
	}
	return pulses, xq, int(cSeed), sltp, sltpRaw, delayedGain, finalState
}
