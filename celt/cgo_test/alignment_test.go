...
func TestDecodedAlignmentAnalysis(t *testing.T) {
	sampleRate := 48000
	frameSize := 960
	...
	libDecoded, samples := libDec.DecodeFloat(libPacket, frameSize)
	if samples <= 0 {
	...
	for delay := -500; delay <= 500; delay++ {
		corr := computeCrossCorrelation(pcm, float64Slice2(libDecoded[:frameSize]), delay)
		if corr > bestCorr {
			bestCorr = corr
			bestDelay = delay
		}
	...
	for i := 200; i < 210; i++ {
		...
		if decIdx >= 0 && decIdx < frameSize {
			t.Logf("  [%d] orig=%.5f, decoded=%.5f, diff=%.5f",
				i, pcm[i], libDecoded[decIdx], pcm[i]-float64(libDecoded[decIdx]))
		}
	}
	...
	for i := 120; i < frameSize-120; i++ {
		signalPower += pcm[i] * pcm[i]
		noise := pcm[i] - float64(libDecoded[i])
		noisePower += noise * noise
	}
...
func float64Slice2(f32 []float32) []float64 {
...
func computeCrossCorrelation(x, y []float64, delay int) float64 {
	n := len(x)
	if len(y) < n {
		n = len(y)
	}
