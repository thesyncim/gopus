...
func TestAMD64DispatchMatchesGeneric(t *testing.T) {
	...
	if !reflect.DeepEqual(xcorrGot, xcorrWant) {
		t.Fatalf("celtPitchXcorr mismatch: got %v want %v", xcorrGot, xcorrWant)
	}
	...
	if got, want := prefilterInnerProd(prefX, prefY1, len(prefX)), prefilterInnerProdGeneric(prefX, prefY1, len(prefX)); got != want {
		t.Fatalf("prefilterInnerProd mismatch: got %v want %v", got, want)
	}
	...
	if !reflect.DeepEqual(rotGot, rotWant) {
		t.Fatalf("expRotation1Stride2 mismatch: got %v want %v", rotGot, rotWant)
	}
	...
	if gotMean != wantMean || !reflect.DeepEqual(transientGot, transientWant) {
		t.Fatalf("transientEnergyPairs mismatch: got (%v,%v) want (%v,%v)", gotMean, transientGot, wantMean, transientWant)
	}
	...
	if gotR00 != wantR00 || gotR01 != wantR01 || gotR02 != wantR02 {
		t.Fatalf("toneLPCCorr mismatch: got (%v,%v,%v) want (%v,%v,%v)", gotR00, gotR01, gotR02, wantR00, wantR01, wantR02)
	}
	...
	if !reflect.DeepEqual(prefXcorrGot, prefXcorrWant) {
		t.Fatalf("prefilterPitchXcorr mismatch: got %v want %v", prefXcorrGot, prefXcorrWant)
	}
}
