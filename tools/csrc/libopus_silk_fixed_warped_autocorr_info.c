/* Oracle for the libopus FIXED_POINT silk_warped_autocorrelation_FIX kernel.
 *
 ...
 * cases from stdin and writes the bit-exact scale + corr[] results to stdout. */
...
#define INPUT_MAGIC "GWAI"
#define OUTPUT_MAGIC "GWAO"

#define MAX_LEN 4096
#define MAX_ORDER 24 /* MAX_SHAPE_LPC_ORDER */

void silk_warped_autocorrelation_FIX_c(opus_int32 *corr, opus_int *scale,
...
int main(void) {
  ...
  for (uint32_t c = 0; c < count; c++) {
    int32_t warping_Q16, order;
    uint32_t len;
    if (!read_i32(&warping_Q16) || !read_u32(&len) || !read_i32(&order)) {
      return 1;
    }
    if (len > MAX_LEN || order < 0 || order > MAX_ORDER || (order & 1)) {
      return 1;
    }

    static int16_t in[MAX_LEN];
    opus_int32 corr[MAX_ORDER + 1];
    opus_int scale = 0;
    ...
    silk_warped_autocorrelation_FIX_c(corr, &scale, in, (opus_int)warping_Q16,
                                      (opus_int)len, (opus_int)order);

    if (!write_i32((int32_t)scale)) return 1;
    for (int32_t i = 0; i < order + 1; i++) {
      if (!write_i32((int32_t)corr[i])) return 1;
    }
  }
