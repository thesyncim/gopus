...
int main(void) {
  ...
  ret = opus_dred_parse(dred_dec, dred, packet, (opus_int32)packet_len, (opus_int32)max_dred_samples,
                        (opus_int32)sample_rate, &dred_end, 1);

  if (!write_exact(GODO_MAGIC, 4) || !write_u32(1) || !write_i32(ret) || !write_i32(dred_end) ||
      !write_i32(dred->process_stage) || !write_i32(dred->nb_latents) || !write_i32(dred->dred_offset)) {
