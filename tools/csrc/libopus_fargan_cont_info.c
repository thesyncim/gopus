...
#define INPUT_MAGIC "GFC0"
#define OUTPUT_MAGIC "GFO0"
#define NB_FEATURES 20
#define CONT_FEATURES (5 * NB_FEATURES)
...
int main(void) {
  char magic[4];
  uint32_t version;
  float pcm0[FARGAN_CONT_SAMPLES];
  float features0[CONT_FEATURES];
  ...
  if (!read_bits_array(pcm0, FARGAN_CONT_SAMPLES) || !read_bits_array(features0, CONT_FEATURES)) {
    fprintf(stderr, "failed to read fargan continuity payload\n");
    return 1;
  }

  fargan_init(&st);
  fargan_cont(&st, pcm0, features0);
  ...
  if (!write_exact(OUTPUT_MAGIC, 4) ||
    ...
    fprintf(stderr, "failed to write fargan continuity header\n");
    return 1;
  }
  if (!write_bits_array(&st.deemph_mem, 1) ||
    ...
    fprintf(stderr, "failed to write fargan continuity state\n");
    return 1;
  }
  return 0;
}
