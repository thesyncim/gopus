/* Oracle for the libopus FIXED_POINT silk_LPC_analysis_filter kernel.
 *
 ...
 * cases from stdin and writes the bit-exact int16 filter outputs to stdout. */
...
int main(void) {
  ...
  for (uint32_t c = 0; c < count; c++) {
    uint32_t d, len;
    if (!read_u32(&d) || !read_u32(&len)) return 1;
    if (len > MAX_LEN || d > SILK_MAX_ORDER_LPC) return 1;

    int16_t B[SILK_MAX_ORDER_LPC];
    static int16_t in[MAX_LEN];
    static int16_t out[MAX_LEN];

    for (uint32_t i = 0; i < d; i++) {
      if (!read_i16(&B[i])) return 1;
    }
    for (uint32_t i = 0; i < len; i++) {
      if (!read_i16(&in[i])) return 1;
    }

    silk_LPC_analysis_filter(out, in, B, (opus_int32)len, (opus_int32)d, 0);
