/* Oracle for the libopus FIXED_POINT silk_schur + silk_k2a kernels.
 *
 ...
 * cases from stdin and writes the bit-exact residual energy, reflection
 * coefficients, and LPC coefficients to stdout. */
...
int main(void) {
  ...
  for (uint32_t cidx = 0; cidx < count; cidx++) {
    ...
    opus_int32 res = silk_schur(rc_Q15, c, (opus_int32)order);

    for (int32_t i = 0; i < order; i++) {
      A_Q24[i] = 0;
    }
    silk_k2a(A_Q24, rc_Q15, (opus_int32)order);

    if (!write_i32((int32_t)res)) return 1;
    for (int32_t i = 0; i < order; i++) {
      if (!write_i32((int32_t)rc_Q15[i])) return 1;
    }
    for (int32_t i = 0; i < order; i++) {
