...
#define INPUT_MAGIC "GSPN"
#define OUTPUT_MAGIC "GSPQ"

void celt_pitch_xcorr_float_neon(const opus_val16 *x, const opus_val16 *y,
...
static int eval_record(void) {
  ...
  celt_pitch_xcorr_float_neon(x, y, out, (int)length, (int)max_pitch, 0);
  if (!write_u32(max_pitch)) return 0;
  for (i = 0; i < max_pitch; i++) {
    memcpy(&raw, &out[i], sizeof(out[i]));
...
int main(void) {
  ...
  if (!read_u32(&version) || version != 1 || !read_u32(&mode) || mode != 0 || !read_u32(&count)) return 1;
