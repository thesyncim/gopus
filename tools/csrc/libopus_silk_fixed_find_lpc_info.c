/* Oracle for the libopus FIXED_POINT silk_find_LPC_FIX driver.
 *
 ...
 * silk_encoder_state, runs the real silk_find_LPC_FIX, and writes the bit-exact
 * NLSF_Q15 vector and the chosen NLSFInterpCoef_Q2 to stdout. */
...
void silk_find_LPC_FIX(silk_encoder_state *psEncC, opus_int16 NLSF_Q15[],
                       const opus_int16 x[], const opus_int32 minInvGain_Q30);
...
int main(void) {
  ...
  for (uint32_t c = 0; c < count; c++) {
    int32_t predictLPCOrder, subfr_length, nb_subfr, useInterp, firstFrame;
    int32_t minInvGain_Q30;
    int16_t prevNLSF[MAX_LPC_ORDER];
    uint32_t x_len;

    if (!read_i32(&predictLPCOrder) || !read_i32(&subfr_length) ||
        !read_i32(&nb_subfr) || !read_i32(&useInterp) ||
        !read_i32(&firstFrame) || !read_i32(&minInvGain_Q30)) {
      return 1;
    }
    ...
    psEncC.predictLPCOrder = predictLPCOrder;
    psEncC.subfr_length = subfr_length;
    psEncC.nb_subfr = nb_subfr;
    psEncC.useInterpolatedNLSFs = useInterp;
    psEncC.first_frame_after_reset = firstFrame;
    psEncC.arch = 0;
    for (int i = 0; i < MAX_LPC_ORDER; i++) {
      psEncC.prev_NLSFq_Q15[i] = prevNLSF[i];
    }

    opus_int16 NLSF_Q15[MAX_LPC_ORDER];
    memset(NLSF_Q15, 0, sizeof(NLSF_Q15));

    silk_find_LPC_FIX(&psEncC, NLSF_Q15, xbuf, minInvGain_Q30);

    for (int i = 0; i < MAX_LPC_ORDER; i++) {
      if (!write_i32((int32_t)NLSF_Q15[i])) return 1;
    }
    if (!write_i32((int32_t)psEncC.indices.NLSFInterpCoef_Q2)) return 1;
  }
