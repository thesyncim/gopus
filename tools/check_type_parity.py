...
The codec currently has known legacy scalar-width debt, including float64,
complex128, 64-bit FFT helpers, and transitional scratch allocators. This guard
makes that debt explicit and ratchets it down: current findings must match the
...
def write_allowlist(path: Path, findings: dict[FindingKey, Finding], reason: str) -> None:
    ...
    lines = [
        ...
        "# Agents: do not update this file to hide new guarded type-parity debt.",
...
def report_mismatches(
    ...
    if new_or_grown:
        print("\nNew or increased runtime type-parity debt:", file=sys.stderr)
