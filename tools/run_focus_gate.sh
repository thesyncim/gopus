...
gate_extra_controls_tag() {
	...
	run_parity -tags gopus_extra_controls ./internal/dred -run 'Test(EncodeExperimentalPayloadMatchesLibopus|EncodeExperimentalPayloadMatchesLibopusDelayedOffset)' -count=1
...
gate_extra_controls_parity() {
	...
	run_parity -tags gopus_extra_controls ./internal/dred -run "$dred_reference_internal" -count=1
	run_parity -tags gopus_extra_controls ./internal/dred/rdovae -run "$dred_reference_rdovae" -count=1
	run_parity -tags gopus_extra_controls ./internal/dred -run "Test(ConvertTo16kMonoFloat64MatchesLibopus|ConvertTo16kMonoFloat64MatchesLibopusAcrossCalls|EncodeExperimentalPayloadMatchesLibopusLargeLaplaceContinuation)|$dred_rdovae_real_model" -count=1
	run_parity -tags gopus_extra_controls ./internal/dred -run "$dred_latents_trace" -count=1
	run_parity -tags gopus_extra_controls ./internal/lpcnetplc -run 'Test(LPCNetSingleFrameFeaturesFloatMatchesLibopusColdStart|LPCNetSingleFrameFeaturesFloatMatchesLibopusStatefulSequence|BurgCepstralAnalysisMatchesLibopus|PitchDNNMatchesLibopusOnRealModel|ConcealFrameFloatWithAnalysisMatchesLibopus)' -count=1
