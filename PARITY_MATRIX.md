...
| Opus Custom | optional custom-mode API | **T** `gopus_custom` (≡ libopus `CUSTOM_MODES`) — `celt/custom`: `NewMode`/`NewEncoder`/`NewDecoder` + custom CTLs; standard modes byte-identical to libopus (CI-mandatory `make test-custom-parity` against the `--enable-custom-modes` oracle on Linux + macOS), zero-cost in default builds | implemented (tagged) | Non-standard-rate oracle parity still needs a real custom-mode CELT path (roundtrip self-consistent today; `GOPUS_CUSTOM_NONSTANDARD_STRICT=1` flips the gap to a hard failure once it lands) |
...
3. **Opus Custom non-standard-rate oracle parity** — `gopus_custom` standard modes are byte-identical to libopus and now CI-mandatory via `make test-custom-parity` against the `--enable-custom-modes` oracle; non-standard-rate custom modes are only roundtrip self-consistent and need a real custom-mode CELT path before they match that oracle build.  
