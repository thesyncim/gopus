...
evidence: In `testvectors/encoder_compliance_test.go`, the hybrid SWB row is now stable at `gap=0.00 dB` in fixture-backed compliance runs, so its prior `-0.05 dB` override was stale. After removal, compliance/parity/provenance gates remained green: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderComplianceSummary -count=1 -v`, `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderCompliancePrecisionGuard -count=1 -v`, `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`. `make verify-production` showed unchanged known local blocker at `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: hybrid SWB 20ms compliance gap turns negative again under fixture-backed parity, compliance decode/quality semantics change, or refreshed libopus references materially shift this lane.
owner: codex
...
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), hybrid SWB 40ms packetization/control flow, or cross-OS CI evidence changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: variants ratchet floor hardening for SILK-WB-40ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-40ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.04` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.08` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches across 3 runs each: arm64 `gap=-0.00 dB` and amd64 (`GOARCH=amd64`) `gap=0.00 dB`, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-40ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 40ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: variants ratchet floor hardening for SILK-WB-60ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-60ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.04` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.08` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches across 3 runs each: arm64 `gap=0.00 dB` and amd64 (`GOARCH=amd64`) `gap=0.00 dB`, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-60ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 60ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: variants ratchet floor hardening for SILK-WB-60ms mono impulse provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-60ms-mono-32k|impulse_train_v1` at `min_gap_db=-0.05` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.10` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches across 3 runs each: arm64 `gap=-0.04 dB` and amd64 (`GOARCH=amd64`) `gap=-0.00 dB`, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-60ms-mono-32k-impulse_train_v1$' -count=3 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderComplianceSummary -count=1 -v`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 60ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex
...
decision: Keep tightened amd64 variant ratchet floor for `HYBRID-SWB-40ms-mono-48k|impulse_train_v1` at `min_gap_db=-0.39` in `encoder_compliance_variants_ratchet_baseline_amd64.json`, while keeping default baseline floor unchanged at `-0.09`.
evidence: Focused repeated amd64 lane checks were stable at `gap=-0.12 dB` across 3 runs, with `mismatch=0.00%` and `histL1=0.000`, using `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-40ms-mono-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), hybrid SWB 40ms packetization/control flow, or cross-OS CI evidence changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: amd64-only ratchet floor hardening for SILK-WB-20ms stereo impulse provenance lane
decision: Keep tightened amd64 variant ratchet floor for `SILK-WB-20ms-stereo-48k|impulse_train_v1` at `min_gap_db=-0.12` in `encoder_compliance_variants_ratchet_baseline_amd64.json`, while keeping default baseline floor unchanged at `-0.09`.
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.08 dB` and amd64 (`GOARCH=amd64`) `gap=-0.04 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-20ms-stereo-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), SILK WB stereo 20ms packetization/control flow, or cross-OS CI evidence changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: amd64-only ratchet floor hardening for HYBRID-SWB-20ms am-multisine provenance lane
decision: Keep tightened amd64 variant ratchet floor for `HYBRID-SWB-20ms-mono-48k|am_multisine_v1` at `min_gap_db=-0.45` in `encoder_compliance_variants_ratchet_baseline_amd64.json`, while keeping default baseline floor unchanged at `-0.06`.
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.04 dB` and amd64 (`GOARCH=amd64`) `gap=-0.10 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-20ms-mono-48k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), hybrid SWB 20ms packetization/control flow, or cross-OS CI evidence changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: Variants ratchet floor hardening for HYBRID-SWB-40ms impulse provenance lane
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-40ms-mono-48k|impulse_train_v1` at `min_gap_db=-0.09` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.40` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.09 dB` and amd64 (`GOARCH=amd64`) `gap=-0.12 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-40ms-mono-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), hybrid SWB 40ms packetization/control flow, or cross-OS CI evidence changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-28
topic: Variants ratchet floor hardening for SILK-WB-20ms stereo impulse provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-20ms-stereo-48k|impulse_train_v1` at `min_gap_db=-0.09` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.15` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.08 dB` and amd64 (`GOARCH=amd64`) `gap=-0.04 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-20ms-stereo-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
...
date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-10ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-10ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.03` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.10` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=0.00 dB` and amd64 (`GOARCH=amd64`) `gap=0.00 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-10ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 10ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-20ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-20ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.03` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.10` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.00 dB` and amd64 (`GOARCH=amd64`) `gap=-0.00 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-20ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 20ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-40ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-40ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.05` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.15` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.00 dB` and amd64 (`GOARCH=amd64`) `gap=0.00 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-40ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 40ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex
...
date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-60ms mono am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-60ms-mono-32k|am_multisine_v1` at `min_gap_db=-0.05` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.15` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=0.00 dB` and amd64 (`GOARCH=amd64`) `gap=0.00 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-60ms-mono-32k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 60ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex
...
date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-60ms mono impulse provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-60ms-mono-32k|impulse_train_v1` at `min_gap_db=-0.08` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.20` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.04 dB` and amd64 (`GOARCH=amd64`) `gap=-0.00 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-60ms-mono-32k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB 60ms mono packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-27
topic: Variants ratchet floor hardening for HYBRID-SWB-20ms am-multisine provenance lane
decision: Keep tightened variant ratchet floor for `HYBRID-SWB-20ms-mono-48k|am_multisine_v1` at `min_gap_db=-0.06` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) while keeping the amd64 baseline floor at `min_gap_db=-0.50` (`encoder_compliance_variants_ratchet_baseline_amd64.json`) per prior windows CI stability evidence.
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.04 dB` and amd64 (`GOARCH=amd64`) `gap=-0.10 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-20ms-mono-48k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or hybrid SWB 20ms encoder packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-20ms stereo impulse provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-20ms-stereo-48k|impulse_train_v1` at `min_gap_db=-0.10` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.25` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.08 dB` and amd64 (`GOARCH=amd64`) `gap=-0.04 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/SILK-WB-20ms-stereo-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or SILK WB stereo 20ms encoder packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex

date: 2026-02-27
topic: Variants ratchet floor hardening for HYBRID-SWB-40ms impulse provenance lane
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-40ms-mono-48k|impulse_train_v1` at `min_gap_db=-0.10` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.45` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on local arches: arm64 `gap=-0.09 dB` and amd64 (`GOARCH=amd64`) `gap=-0.12 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-40ms-mono-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). An initial amd64 floor of `-0.30` failed required windows CI on PR #234 (`test-windows`) with `gap=-0.37 dB`, so amd64 was recalibrated to `-0.45` to preserve cross-OS stability while keeping a tighter floor than `-0.50`. Full validation passed after recalibration: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
...
date: 2026-02-27
topic: Variants ratchet floor hardening for SILK-WB-20ms stereo impulse provenance lane
decision: Keep tightened variant ratchet floors for `SILK-WB-20ms-stereo-48k|impulse_train_v1` at `min_gap_db=-0.12` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.35` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
...
date: 2026-02-27
topic: Variants ratchet floor hardening for HYBRID-SWB-40ms impulse provenance lane
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-40ms-mono-48k|impulse_train_v1` at `min_gap_db=-0.12` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.50` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
evidence: Focused repeated lane checks were stable on both arches: arm64 `gap=-0.09 dB` and amd64 (`GOARCH=amd64`) `gap=-0.12 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, using `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-40ms-mono-48k-impulse_train_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Full validation passed after tightening: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or hybrid SWB 40ms encoder packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex
...
date: 2026-02-27
topic: Variants ratchet floor hardening for HYBRID-SWB-20ms am-multisine provenance lane
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-20ms-mono-48k|am_multisine_v1` at `min_gap_db=-0.08` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.50` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`). Do not keep stale amd64 floor `-9.135...` for this lane.
evidence: Focused repeated lane checks were stable locally on both arches: arm64 `gap=-0.04 dB` and amd64 (`GOARCH=amd64`) `gap=-0.10 dB` across 3 runs each, with `mismatch=0.00%` and `histL1=0.000`, via `GOPUS_TEST_TIER=parity go test ./testvectors -run 'TestEncoderVariantProfileParityAgainstLibopusFixture/cases/HYBRID-SWB-20ms-mono-48k-am_multisine_v1$' -count=1 -v` (expected parent-level `ratchet baseline coverage mismatch` on single-case invocation). Initial amd64 floor `-0.25` failed required windows CI (`test-windows`, PR #231) where this lane measured `gap=-0.43 dB`; recalibrating to `-0.50` restored cross-OS safety while remaining much tighter than the stale `-9 dB` baseline. Local validation after recalibration passed: `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1 -v`, `GOARCH=amd64 GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderVariantProfileParityAgainstLibopusFixture -count=1 -v`, `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestEncoderVariantProfileProvenanceAudit -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
do_not_repeat_until: fixture corpus, quality scoring path (`qualityFromPacketsLibopusReference*` / `ComputeQualityFloat32WithDelay`), or hybrid SWB 20ms encoder packetization/control flow changes and produces a materially different stable gap distribution for this lane.
owner: codex
...
decision: Keep a dedicated decoder-loss stress ratchet for `celt-fb-20ms-mono-64k-plc|periodic5` in `testvectors/decoder_loss_parity_test.go` with tightened `minQ=82.0`, `minCorr=0.999`, and RMS ratio bounds `[0.995, 1.005]` to guard the current weakest CELT stress lane against regressions while retaining realistic concealment tolerance.
evidence: Tightened ratchet entry in `decoderLossStressThresholdForCase` from `minQ=80.0` to `82.0` after repeated stable focused measurements on both arches: arm64 `Q=82.87` and amd64 (`GOARCH=amd64`) `Q=83.99` across 3 runs each (`GOPUS_TEST_TIER=exhaustive go test ./testvectors -run 'TestDecoderLossStressPatternsAgainstOpusDemo/celt-fb-20ms-mono-64k-plc/periodic5$' -count=1 -v`, repeated). Validation passed with the tightened floor: `GOPUS_TEST_TIER=exhaustive go test ./testvectors -run TestDecoderLossStressPatternsAgainstOpusDemo -count=1 -v`, `GOPUS_TEST_TIER=parity go test ./testvectors -run TestDecoderLossParityLibopusFixture -count=1 -v`, and `make bench-guard`; `make verify-production` remained blocked only by existing `tmp_check` cgo-disabled setup while non-`tmp_check` packages passed.
