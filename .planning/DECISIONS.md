...
date: 2026-03-08
topic: amd64 CELT asm build selection and default-on arm64 asm
decision: Keep CELT amd64 ISA selection at build time using Go's standard microarchitecture tags instead of runtime CPU probes or custom user-facing asm tags. Baseline-safe amd64 kernels stay on their normal `amd64` wrappers, AVX/FMA-class kernels live behind `amd64.v3`, and `amd64 && !amd64.v3` uses exact generic helpers for those surfaces. Keep arm64 asm default-on with no extra split because the current arm64 kernels only use baseline ISA. Do not reintroduce `golang.org/x/sys/cpu` runtime dispatch or hidden asm-enablement tags for these paths.
evidence: Replaced `celt/amd64_dispatch.go` with `celt/amd64_dispatch_helpers.go`, `celt/amd64_dispatch_v1.go`, and `celt/amd64_dispatch_v3.go`; retagged the AVX/FMA amd64 `.s` files to `//go:build amd64.v3`; updated `celt/amd64_dispatch_test.go` to compare the active path directly against the generic helpers; and removed `golang.org/x/sys` from `go.mod`/`go.sum`. Validation passed: `go test ./celt -count=1`, `GOARCH=amd64 GOAMD64=v1 go test ./celt -run '^TestAMD64DispatchMatchesGeneric$' -count=1`, `GOARCH=amd64 GOAMD64=v1 go test ./celt -count=1`, `GOARCH=amd64 GOAMD64=v3 go test ./celt -run '^$' -c -o /tmp/celt_amd64v3.test`, `GOPUS_TEST_TIER=parity go test ./testvectors -run TestEncoderComplianceSummary -count=1 -v` (`23 passed, 0 failed`), and `make bench-guard`.
do_not_repeat_until: the shipped amd64/arm64 kernel inventory changes, a kernel starts requiring a different minimum ISA level, or a new exact higher-ISA implementation merits another source-selection split.
owner: codex
