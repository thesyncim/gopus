...
date: 2026-03-11
topic: ARM64 stereo analysis down2 fast path and MDCT post-twiddle index walk
decision: Keep the arm64 runtime fast path for `silkResamplerDown2HPStereo()` and the running-index MDCT post-twiddle writeback loops in `celt/mdct_encode.go`. Preserve the current helper dispatch split (`analysis_resampler_stereo_arm64.go` / `analysis_resampler_stereo_other.go`) and the current `lo/hi` coefficient walk unless same-host encode A/B stops favoring them.
evidence: Focused exactness passed with `GOWORK=off go test ./encoder -run '^(TestSilkResamplerDown2HPStereoMatchesDownmixThenResample|TestSilkResamplerDown2HPStereoCurrentMatchesGenericExact|TestSilkResamplerDown2HPMatchesLegacy|TestAnalysisTraceFixtureParityWithLibopus)$' -count=1` and `GOWORK=off go test ./celt -run '^(TestMDCTForwardOverlapDirectStageMatchesLegacyStagedPath|TestMDCTShortBlocksCoreMatchesLegacy|TestMDCTForwardWithOverlap.*|TestMDCT.*|TestLibopus.*MDCT.*)$' -count=1`. Broad validation stayed green with `GOPUS_TEST_TIER=parity GOWORK=off go test ./testvectors -run TestEncoderComplianceSummary -count=1 -v` (`23 passed, 0 failed`) and `GOWORK=off make bench-guard`. Same-host A/B versus detached `origin/master` improved the fair encode rows from speech `gopus avg 2.312815916s` to `2.300694625s` and stereo `9.972987028s` to `9.954578513s`. Direct benches favored the keep as well: `BenchmarkSilkResamplerDown2HPStereoCurrent ~1245-1252 ns/op` versus generic `~1423-1432 ns/op`, `BenchmarkTonalityAnalysis48kStereo ~6784-6862 ns/op -> ~6504-6530 ns/op`, and `BenchmarkEncoderEncode_Stereo ~78641-79262 ns/op -> ~77503-77800 ns/op`.
do_not_repeat_until: the 48 kHz stereo tonality-analysis downmix/down2 control flow changes away from the current helper contract, the arm64 compiler/codegen changes enough that the fused-generic sequence no longer matches this asm path exactly, or the MDCT post-twiddle coefficient layout/order changes so the current `lo/hi` walk is no longer the shipped output pattern.
owner: codex
