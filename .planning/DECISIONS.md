...
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-20ms-mono-48k|am_multisine_v1` to reflect current stable parity behavior: `min_gap_db=-0.08` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.25` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`). Do not keep stale amd64 floor `-9.135...` for this lane.
