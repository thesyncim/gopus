...
decision: Keep tightened variant ratchet floors for `SILK-WB-60ms-mono-32k|impulse_train_v1` at `min_gap_db=-0.06` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.12` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
