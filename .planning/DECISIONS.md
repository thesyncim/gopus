...
date: 2026-03-14
topic: Decoder benchmark warm-up before ResetTimer
decision: Keep the decoder benchmark harness (`benchmark_alloc_test.go`) on the current prewarm shape: run one decode before `ResetTimer()`, then force `runtime.GC()` and `runtime.Gosched()` before the timed loop for the float32/int16 decode benchmarks. Treat the prior short-run `1 B/op` / `0 allocs/op` signal as benchmark-startup noise rather than a production decode-path allocation unless an isolated long-run bench or alloc profile reproduces a real decode-owned heap hit.
evidence: An isolated long-run decode bench was already clean (`GOWORK=off go test . -run '^$' -bench '^BenchmarkDecoderDecode_CELT$' -benchmem -count=1 -benchtime=2s -cpu=1 -memprofile /tmp/decoder-celt.mem` -> `0 B/op`, `0 allocs/op`), but the exact short benchguard shape reproduced the stray byte (`GOWORK=off go test . -run '^$' -bench '^BenchmarkDecoderDecode_CELT$' -benchmem -count=5 -benchtime=200ms -cpu=1` -> repeated `1 B/op`). The short alloc profile pointed at runtime startup (`runtime.allocm` / `runtime.mallocgc`) rather than decoder code. After adding the prewarm plus `runtime.GC()` / `runtime.Gosched()`, both the exact short repro for `BenchmarkDecoderDecode_CELT` and `BenchmarkDecoderDecodeInt16` and the full `make bench-guard`/`make verify-production` embedded guard reported `0 B/op`, `0 allocs/op`.
do_not_repeat_until: the benchguard invocation shape changes materially, decoder startup semantics change, or an isolated long-run decode bench/alloc profile starts reproducing a real decode-owned heap allocation that the benchmark warm-up would otherwise mask.
owner: codex

date: 2026-03-14
topic: MDCT post-twiddle running output indices
decision: Keep the `celt/mdct_encode.go` `mdctForwardOverlapF32Scratch()` post-twiddle loops on the `trigHi := trig[n4:]` plus running `lo/hi` output-index shape for both the direct-kiss and fallback paths. This removes repeated `n4+i`, `2*i`, and `n2-1-2*i` address arithmetic while preserving the exact float math and coefficient ordering, so future MDCT perf work should treat this as the new baseline rather than restoring the recomputed-index form.
evidence: Exactness stayed green with `GOWORK=off go test ./celt -run '^(TestMDCTForwardOverlapDirectStageMatchesLegacyStagedPath|TestMDCTShortBlocksCoreMatchesLegacy|TestMDCTUnit_GoMDCT)$' -count=1`. Same-session A/B against `/tmp/gopus-baseline-loop4` favored the keep across the focused MDCT microbench: `BenchmarkMDCTForward` current `413.8/745.8/1445/2921 ns` versus baseline `420.0/754.2/1464/2979 ns` for frame sizes `120/240/480/960`. Root encode stayed effectively flat while stereo improved slightly (`BenchmarkEncoderEncode ~43659 ns/op` current vs `~43579 ns/op` baseline; `BenchmarkEncoderEncode_Stereo ~73900 ns/op` current vs `~74589 ns/op` baseline). Broad gates also stayed green with `make verify-production` and standalone `make bench-guard` (`BenchmarkEncoderEncode ~44123 ns/op`, `BenchmarkEncoderEncodeInt16 ~44464 ns/op`, both `0 allocs/op`).
do_not_repeat_until: MDCT post-twiddle output layout, trig-table slicing, or direct-kiss/fallback store order changes enough that the current `lo/hi` write pattern is no longer the active hot path, or same-host A/B on the target compiler/host stops favoring it over the recomputed-index form.
owner: codex

date: 2026-03-14
topic: Transient-analysis warmup-loop split
decision: Do not split `celt/transient.go` `transientAnalysisScratch()` into a dedicated warmup loop purely to remove the `i < warmupPairs` branch from the main mono/stereo passes. On the current Apple M4 Max host that rewrite preserved exactness but regressed the stereo transient microbench and the root encode row, so future transient-analysis perf loops should look for a different hotspot shape instead of retrying this branch-removal tactic.
evidence: The prototype stayed exact under `GOWORK=off go test ./celt -run '^(TestTransientAnalysisMatchesLegacy|TestRunPrefilterParityAgainstLibopusFixture)$' -count=1`, but same-host A/B against a clean `origin/master` worktree rejected it: `BenchmarkTransientAnalysisCurrent ~3544 ns/op` current vs `~3568 ns/op` baseline (mono marginal), `BenchmarkTransientAnalysisCurrentStereo ~4337 ns/op` current vs `~4286 ns/op` baseline (stereo slower), and root `BenchmarkEncoderEncode ~42045 ns/op` current vs `~41702 ns/op` baseline. The change was reverted immediately.
do_not_repeat_until: transient-analysis warmup semantics, HP-filter control flow, or compiler/host behavior changes enough that this exact branch-removal shape is no longer comparable, or a materially different same-host A/B shows a clear win.
owner: codex
...
decision: Do not reintroduce either of the "cheap" encoder front-end rewrites that only move existing work around without changing the higher-level hotspot shape: (1) writing pre-emphasis directly into the transient-analysis suffix instead of using `applyPreemphasisWithScalingScratch()` plus `copy()`, and (2) hoisting the remaining fixed local arrays in `TonalityAnalysisState.tonalityAnalysis()` into persistent state-backed scratch. On the current Apple M4 Max host both rewrites preserved behavior but regressed the targeted benches and the root encode row, so future front-end perf loops should prefer new hotspot-shape changes over these storage/layout-only rewrites unless there is fresh cross-host evidence.
evidence: Same-host focused reruns on `codex/perf-loop-post-safety` rejected both candidates. The direct-write preemphasis attempt regressed `BenchmarkTransientAnalysisCurrent ~3653 ns/op -> ~3929 ns/op`, `BenchmarkTransientAnalysisCurrentStereo ~4531 ns/op -> ~4812 ns/op`, and root `BenchmarkEncoderEncode ~44133 ns/op baseline profile run -> ~45583 ns/op`. The tonality local-scratch hoist regressed `BenchmarkTonalityAnalysis48kMono ~6458 ns/op -> ~6683 ns/op`, `BenchmarkTonalityAnalysis48kStereo ~6753 ns/op -> ~6930 ns/op`, and root `BenchmarkEncoderEncode ~45520 ns/op -> ~45617 ns/op`. Both candidates were reverted immediately; only the new `celt/pitch_downsample_test.go` parity guard was kept from the loop.
do_not_repeat_until: a materially different host/compiler shows the storage rewrite winning in same-host A/B, `tonalityAnalysis()` or transient-analysis buffer ownership changes enough that these exact shapes are no longer comparable, or a broader algorithmic change makes one of these rewrites part of a new hotspot-shape improvement rather than a standalone micro-optimization.
owner: codex

date: 2026-03-13
topic: Pitch-downsample legacy parity guard
decision: Keep `celt/pitch_downsample_test.go` as an exact regression guard that compares `pitchDownsample()` against the legacy/reference implementation across randomized mono and stereo factor-2 inputs. Treat this as the required guard before future `pitchDownsample()` or `celtFIR5()` performance experiments, because the latest unroll attempt stayed bit-exact but regressed badly on Apple silicon.
evidence: `GOWORK=off go test ./celt -run '^(TestPitchDownsampleMatchesLegacy|TestPrefilterPitchXcorr|TestRemoveDoublingMatchesLegacy|TestRunPrefilterLibopusFixtures)$' -count=1` passed after adding the new randomized parity guard. During the same loop, a temporary `celtFIR5()` unroll kept exact output but regressed `BenchmarkPitchDownsampleCurrentMono ~1819 ns/op -> ~36841 ns/op`, `BenchmarkPitchDownsampleCurrentStereo ~2143 ns/op -> ~25774 ns/op`, and even blew up root `BenchmarkEncoderEncode` into millisecond-scale slowdown, so the perf change was reverted while the guard stayed.
do_not_repeat_until: `pitchDownsample()` stops depending on the current factor-2 path and legacy-reference semantics, or a future libopus-backed fixture replaces this direct legacy implementation as the stronger oracle for the helper.
owner: codex
