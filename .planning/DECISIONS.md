...
decision: Keep tightened variant ratchet floors for `HYBRID-SWB-40ms-mono-48k|impulse_train_v1` at `min_gap_db=-0.10` on default baseline (`encoder_compliance_variants_ratchet_baseline.json`) and `min_gap_db=-0.30` on amd64 baseline (`encoder_compliance_variants_ratchet_baseline_amd64.json`).
