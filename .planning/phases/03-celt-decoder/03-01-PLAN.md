...
must_haves:
  truths:
    - "CELT decoder struct can be instantiated with channel count"
    - "Frame size configuration available for 2.5/5/10/20ms frames"
    - "Band edge table (eBands) provides correct MDCT bin indices"
    - "Energy prediction coefficients match RFC 6716 values"
  artifacts:
