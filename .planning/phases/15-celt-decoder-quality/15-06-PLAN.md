...
must_haves:
  truths:
    - "Decoder divergence from reference can be identified through trace inspection"
    - "Trace output enables comparison with libopus reference values"
  ...
  key_links:
    ...
    - from: "internal/celt/debug_trace.go"
      ...
      via: "TraceEnergy calls from energy decoding"
      pattern: "TraceEnergy"
    - from: "internal/celt/debug_trace.go"
      to: "internal/celt/bands.go"
      via: "TracePVQ and TraceCoeffs calls from band decoding"
      pattern: "Trace(PVQ|Coeffs)"
    - from: "internal/celt/debug_trace.go"
      to: "internal/celt/pvq.go"
      via: "Trace integration for PVQ vector decoding"
      pattern: "Trace"
---
...
Add helper function boolToInt(b bool) int in decoder.go.
  ...
  <verify>go build ./internal/celt/... compiles without errors AND go test -v ./internal/celt/... -run TestDecodeFrame -count=1 passes (tracing is no-op by default)</verify>
  <done>Trace calls added to decoder.go, energy.go, bands.go, pvq.go. Code compiles and tests pass (tracing disabled by default).</done>
</task>
