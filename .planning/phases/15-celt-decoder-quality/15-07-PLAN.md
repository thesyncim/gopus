...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/celt/bands_test.go"
      provides: "Bit allocation to pulse count tests"
      contains: "TestBitsToK"
  key_links:
    ...
    - from: "internal/celt/bands.go"
      ...
      via: "DecodePVQ call in band decoding"
      pattern: "DecodePVQ"
---
...
<task type="auto">
  ...
  <action>
Add tests to the EXISTING bands_test.go file (which already exists with ~19KB of tests). Extend the existing test coverage with new tests for bit allocation to pulse count conversion.
