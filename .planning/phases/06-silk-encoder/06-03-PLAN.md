...
// First subframe is absolute, subsequent are delta-coded.
// Per RFC 6716 Section 4.2.7.6.
func (e *Encoder) encodePitchLags(pitchLags []int, numSubframes int) {
    config := GetBandwidthConfig(e.bandwidth)

    // Determine pitch contour index (per RFC 6716 Section 4.2.7.6.2)
    // Contour encodes delta pattern for all subframes
    contourIdx, lagOffset := e.findBestPitchContour(pitchLags, numSubframes)

    // Encode absolute lag for first subframe
    // lagOffset is the base lag
    lagIdx := lagOffset - config.PitchLagMin
    if lagIdx < 0 {
        lagIdx = 0
    }
    ...
    // Encode lag MSB and LSB per RFC 6716 Section 4.2.7.6.1
    lagMSB := lagIdx / 4
    lagLSB := lagIdx % 4

    e.rangeEncoder.EncodeICDF16(lagMSB, ICDFPitchLagMSB, 8)
    e.rangeEncoder.EncodeICDF16(lagLSB, ICDFPitchLagLSB, 8)

    // Encode contour index for delta pattern
    e.rangeEncoder.EncodeICDF16(contourIdx, ICDFPitchContour, 8)
}

// findBestPitchContour finds the contour that best matches pitch lag pattern.
// Returns contour index and base lag.
func (e *Encoder) findBestPitchContour(pitchLags []int, numSubframes int) (int, int) {
    // Find mean lag
    var sumLag int
    ...
    // Find best matching contour from PitchContour table
    bestContour := 0
    bestDist := math.MaxInt32

    for cIdx := 0; cIdx < len(PitchContour); cIdx++ {
        contour := PitchContour[cIdx]
        if len(contour) < numSubframes {
            continue
        }
...
func (e *Encoder) encodeLTPCoeffs(ltpCoeffs [][]int8, periodicity int, numSubframes int) {
    ...
    e.rangeEncoder.EncodeICDF16(periodicity, ICDFLTPPeriodicity, 8)

    // Encode LTP scale index
    scaleIdx := 0 // Default scale
    e.rangeEncoder.EncodeICDF16(scaleIdx, ICDFLTPScale, 8)
    ...
    for sf := 0; sf < numSubframes; sf++ {
        ...
        e.rangeEncoder.EncodeICDF16(cbIdx, ICDFLTPIndex[periodicity], 8)
    }
}
