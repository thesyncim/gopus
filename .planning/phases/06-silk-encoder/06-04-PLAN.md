...
must_haves:
  ...
  key_links:
    - from: "internal/silk/gain_encode.go"
      ...
      via: "GainDequantTable for validation, ICDF tables for encoding"
      pattern: "GainDequantTable|ICDFGainMSB|ICDFDeltaGain"
    ...
    - from: "internal/silk/lsf_quantize.go"
      to: "internal/silk/tables.go"
      via: "ICDF tables for LSF encoding"
      pattern: "ICDFLSFStage1|ICDFLSFStage2|ICDFLSFInterpolation"
---
...
#   - ICDFDeltaGain (delta between subframes)
#   - GainDequantTable (Q16 gain values for inverse quantization)
# LSF tables:
#   - ICDFLSFStage1NBMBVoiced, ICDFLSFStage1NBMBUnvoiced (stage 1 NB/MB)
#   - ICDFLSFStage1WBVoiced, ICDFLSFStage1WBUnvoiced (stage 1 WB)
#   - ICDFLSFStage2NBMB, ICDFLSFStage2WB (stage 2 residual tables)
...
IMPORTANT: Use existing tables from tables.go:
- GainDequantTable for inverse quantization lookup
- ICDFGainMSBInactive, ICDFGainMSBUnvoiced, ICDFGainMSBVoiced for MSB encoding
- ICDFGainLSB for LSB encoding
- ICDFDeltaGain for delta encoding between subframes
...
IMPORTANT: Use existing tables from tables.go and codebook.go:
- LSFCodebookNBMB, LSFCodebookWB (stage 1 codebooks from codebook.go)
- LSFStage2ResNBMB, LSFStage2ResWB (stage 2 residuals from codebook.go)
- ICDFLSFStage1NBMBVoiced, ICDFLSFStage1NBMBUnvoiced (stage 1 ICDF for NB/MB)
- ICDFLSFStage1WBVoiced, ICDFLSFStage1WBUnvoiced (stage 1 ICDF for WB)
- ICDFLSFStage2NBMB, ICDFLSFStage2WB (stage 2 ICDF tables)
- ICDFLSFInterpolation (interpolation index)
...
func (e *Encoder) quantizeLSF(lsfQ15 []int16, bandwidth Bandwidth, signalType int) (int, []int, int) {
    ...
    // Select codebook based on bandwidth (from codebook.go)
...
func (e *Encoder) computeStage2Residuals(lsfQ15 []int16, stage1 []uint8, stage1Idx int, isWideband bool, lpcOrder int) []int {
    ...
    // Select residual codebook (from codebook.go)
