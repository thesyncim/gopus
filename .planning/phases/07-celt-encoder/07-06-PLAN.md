...
must_haves:
  truths:
    ...
    - "Signal quality verified via SNR or energy comparison"
  artifacts:
...
2. **writeOggOpus(w io.Writer, packets [][]byte, sampleRate, channels int) error**: Write minimal Ogg Opus file
   ...
   - Each packet gets its own Ogg page for simplicity
   ...
   - CRC checksum (4 bytes LE) - can use crc32 package
...
`go build ./internal/celt/` compiles successfully (helpers are in test file, should not affect main build).
  ...
  <done>
Helper functions exist for writing Ogg Opus files, decoding with opusdec, and comparing waveforms.
  </done>
</task>
...
`go test -v ./internal/celt/... -run Libopus` - shows quality metrics in output.
  ...
  <done>
Cross-validation tests report signal quality metrics (energy ratio, SNR) for debugging.
  </done>
</task>
...
<success_criteria>
- Helper functions exist for Ogg Opus writing and WAV parsing
