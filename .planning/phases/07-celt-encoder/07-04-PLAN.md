...
must_haves:
  truths:
    ...
    - "Stereo encoding produces valid mid-side or dual stereo packets"
  artifacts:
...
2. Create stereo_encode.go:

```go
// EncodeStereoParams decides and encodes stereo mode parameters.
// Returns intensity band (-1 if not using intensity stereo).
func (e *Encoder) EncodeStereoParams(energiesL, energiesR []float64, nbBands int) int
```
...
- Intensity stereo: for correlated bands, encode only mid channel

For initial implementation: always use mid-side, intensity=-1 (disabled).
  </action>
  <verify>`go build ./internal/celt/` compiles</verify>
  <done>Transient detection and stereo encoding implemented</done>
</task>
...
12. Encode coarse energy: e.EncodeCoarseEnergy(energies, nbBands, intra, lm)
13. Compute bit allocation: ComputeAllocation(...)
14. Encode fine energy: e.EncodeFineEnergy(energies, nbBands, fineBits)
15. Encode bands: e.EncodeBands(shapes, bandBits, nbBands, frameSize)
16. Encode energy remainder: e.EncodeEnergyRemainder(nbBands, remainderBits)

17. Finalize: bytes = e.rangeEncoder.Done()
18. Update state: e.SetPrevEnergy(energies)

Return encoded bytes.
  </action>
  <verify>`go build ./internal/celt/` compiles</verify>
  <done>EncodeFrame pipeline implemented</done>
</task>
...
f) TestCELTRoundTripMultipleFrames:
  ...
  <done>Public API and round-trip tests complete, all tests pass</done>
</task>
...
6. Transient detection triggers short blocks when appropriate
7. Multiple consecutive frames maintain state correctly
</verification>

<success_criteria>
- DetectTransient identifies energy jumps > 6dB
- EncodeFrame produces valid CELT packets
