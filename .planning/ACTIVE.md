...
- 2026-02-12: Tightened validation floors across the full summary matrix by ratcheting `encoderLibopusGapFloorDB` upward in `testvectors/encoder_precision_guard_test.go` (general increase of +0.30 dB per profile). Confirmed guard integrity with `go test ./testvectors -run TestEncoderCompliancePrecisionGuard -count=1 -v` PASS and no parity regressions in `go test ./testvectors -run 'TestEncoderComplianceSummary|TestEncoderVariantProfileParityAgainstLibopusFixture' -count=1 -v` PASS.
