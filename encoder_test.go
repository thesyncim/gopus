...
func TestNewEncoder_ValidParams(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			enc, err := NewEncoder(DefaultEncoderConfig(tt.sampleRate, tt.channels, tt.application))
			if err != nil {
				t.Fatalf("NewEncoder(DefaultEncoderConfig(%d, %d, %d)) unexpected error: %v", tt.sampleRate, tt.channels, tt.application, err)
			}
			if enc == nil {
...
func TestNewEncoder_InvalidParams(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			enc, err := NewEncoder(DefaultEncoderConfig(tt.sampleRate, tt.channels, ApplicationAudio))
...
func TestNewEncoder_InvalidApplication(t *testing.T) {
	enc, err := NewEncoder(DefaultEncoderConfig(48000, 1, Application(99)))
...
func TestEncoder_RestrictedApplications(t *testing.T) {
	for _, tt := range restrictedApplicationTestCases() {
		t.Run(tt.name, func(t *testing.T) {
			enc, err := NewEncoder(DefaultEncoderConfig(48000, 1, tt.application))
			if err != nil {
				t.Fatalf("NewEncoder error: %v", err)
			}
...
func TestEncoder_LongPacketRoundTrip(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			enc, err := NewEncoder(DefaultEncoderConfig(48000, 1, tc.application))
			if err != nil {
				t.Fatalf("NewEncoder error: %v", err)
			}
...
func TestEncoder_Lookahead(t *testing.T) {
	for _, tc := range lookaheadTestCases() {
		t.Run(tc.name, func(t *testing.T) {
			enc, err := NewEncoder(DefaultEncoderConfig(tc.sampleRate, 1, tc.application))
			if err != nil {
				t.Fatalf("NewEncoder error: %v", err)
			}
