...
[36001803330](https://github.com/thesyncim/gopus/actions/runs/36001803330): the
...
AMD EPYC 9V74; runs 359822, 359876, 359948, and 360018 use AMD EPYC 7763.
...
| 25 | `xcorrKernelAVX8` | amd64 | `internal/celt/pitch_xcorr_kernel_simd_amd64.go`; `internal/silk/pitch_xcorr_kernel_simd_amd64.go`; `internal/celt/pitch_xcorr_tiny_simd_amd64.go` | archsimd / scalar | CELT coarse L×P=240×360, old asm → Go SIMD: EPYC 9V74 3,447→4,316; EPYC 7763 one-pass 2,454→9,875. Half 480×64: 7763 834.3→3,264. Tiny 5×244: 7763 254.3→230.7. Tiny 10×10: 7763 26.03→165.9. SILK 120×300: 7763 1,198→6,895. Same-run ranges appear below. | 0 | native one-pass production path is exact, allocation-free, and improves long search over the earlier split SIMD loop; long and fine searches still trail assembly; run 360018 fixture gate awaits the captured live-opusdec fixture |
...
| 47 | `xcorrKernelAVX8` | amd64 | `internal/silk/pitch_xcorr_kernel_simd_amd64.go`; `internal/silk/pitch_xcorr_kernel_avx_amd64.go` | archsimd / scalar | SILK pitch search L×P=120×300, old asm → Go SIMD: EPYC 9V74 1,822→2,844; EPYC 7763 one-pass 1,198→6,895; earlier 7763 split SIMD 11,206 | 0 | native one-pass production path is exact and allocation-free; pitch search remains 5.8× slower than assembly on the matched 7763 runner; run 360018 fixture gate awaits the captured live-opusdec fixture |
...
Run 360018 selects the one-pass AMD64 xcorr loop for lengths 120–240.
The same-ISA CBR, focused decode, precision, dispatch, exact xcorr oracle,
and zero-allocation checks pass. The A/B comparator rejects seven skipped
fixture fallback cases and eight fixture-honesty failures: their new packet
hashes are absent from the committed 11-entry Linux AMD64 fixture. The
live `opusdec` capture from that run supplies the review-visible fixture;
the full A/B gate must pass again with that file committed.
...
Run 360018 measures the selected one-pass production path on EPYC 7763 with
...
long CELT xcorr is 22–25% slower on EPYC 9V74 and 3.1–4.8× slower on EPYC
7763. SILK xcorr is 56% slower on 9V74 and 6.4× slower on 7763. The vector
...
7763 regression remains an optimization target.
