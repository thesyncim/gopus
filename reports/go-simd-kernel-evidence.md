...
| 14 | `kfBfly5Inner` | amd64 | `internal/celt/kf_bfly_simd_amd64.go`; `internal/celt/kf_bfly_default.go` | archsimd / scalar | m=8, N=4 on EPYC 7763: old asm 475.9 (475.5–476.3) → Go SIMD 299.7 (298.1–302.2) ns/op; prior scalar Go 537.3 (536.8–540.1) on another runner | 0 | native SIMD is 37% faster than asm in run 360202; bitwise parity and zero-allocation tests pass |
...
| 16 | `kfBfly4Inner` | amd64 | `internal/celt/kf_bfly_simd_amd64.go`; `internal/celt/kf_bfly_default.go` | archsimd / scalar | m=8, N=4 on EPYC 7763: old asm 244.8 (244.0–245.5) → Go SIMD 176.0 (175.7–176.5) ns/op; prior scalar Go 269.9 (269.4–271.9) on another runner | 0 | native SIMD is 28% faster than asm in run 360202; bitwise parity and zero-allocation tests pass |
...
| 25 | `xcorrKernelAVX8` | amd64 | `internal/celt/pitch_xcorr_kernel_simd_amd64.go`; `internal/silk/pitch_xcorr_kernel_simd_amd64.go`; `internal/celt/pitch_xcorr_tiny_simd_amd64.go` | archsimd / scalar | CELT coarse L×P=240×360 old asm→verified one-pass Go SIMD: Xeon 8573C 2,454→9,875; EPYC 7763 3,156→12,077. Half 480×64: Xeon 834.3→3,264; EPYC 1,044→3,243. Tiny 5×244: Xeon 254.3→230.7; EPYC 334.8→239.2. Tiny 10×10 on EPYC 7763 in run 360202: old asm 35.66 (35.46–35.86) → Go SIMD 149.1 (149.1–149.3). SILK 120×300: Xeon 1,198→6,895; EPYC 1,747→7,665. | 0 | verified one-pass long path remains exact and zero alloc; tiny length-10 specialization is 4.2× slower than asm; transposed candidate was reverted after a NaN-payload mismatch and no material coarse-search gain |
...
| 47 | `xcorrKernelAVX8` | amd64 | `internal/silk/pitch_xcorr_kernel_simd_amd64.go`; `internal/silk/pitch_xcorr_kernel_avx_amd64.go` | archsimd / scalar | SILK pitch search L×P=120×300, old asm→verified one-pass Go SIMD: Xeon 8573C 1,198→6,895; EPYC 7763 1,747→7,665; earlier EPYC split SIMD 11,206 | 0 | verified one-pass Go path is exact and zero alloc; transposed candidate was reverted after a NaN-payload mismatch without material long-search gain |
