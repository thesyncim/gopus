...
- A10 extension validation has support work landed: `4c9a5188` made generated DNN model blob headers explicit little-endian through a shared C writer. CELT DRED's retained neural crossfade baseline now uses `celtSig`, OSCE LACE FFT feature scratch is now float32, and OSCE output-scale float-to-int16 quantizers now share a float32 nearest-even helper instead of widening through `math.RoundToEven(float64(...))`. The A10 runtime conversion goal remains open until the remaining OSCE runtime math round-trips, DRED, and LPCNet codec-domain `float64`/`complex128` paths are migrated or source-cited.
...
The guard scans runtime Go files for `float64`, `complex128`, `KissFFT64State`, `ensureFloat64Slice`, and `ensureComplexSlice`, then compares the result with `tools/type_parity_allowlist.tsv`. Current legacy findings are allowed only because they are recorded in that baseline. New findings fail. Removed findings also fail until the baseline is refreshed, so cleanup stays visible in review. As of this checkpoint, local `make test-type-parity` passes with 2025 legacy findings, down from 2029 in the previous checkpoint, 2034 before that, 2068 before that, 2077 before that, 2081 before that, 2083 before that, 2096 before that, 2125 before that, 2144 before that, 2197 before that, 2198 before that, 2207 before that, 2209 before that, 2217 before that, 2220 before that, 2222 before that, 2250 before that, 2273 before that, 2285 before that, 2327 before that, and 2509 in the older baseline.
...
| File | Approx matches |
|---|---:|
| `celt/bands_quant.go` | 245 |
| `celt/encode_frame.go` | 129 |
| `celt/encoder.go` | 98 |
| `celt/mdct.go` | 80 |
| `silk/lpc_analysis.go` | 17 |
| `celt/energy_encode.go` | 64 |
| `celt/postfilter.go` | 63 |
| `celt/mdct_encode.go` | 59 |
| `plc/celt_plc.go` | 52 |
| `celt/channel_adapters.go` | 48 |
| `celt/scratch.go` | 45 |
| `celt/preemph.go` | 44 |
| `celt/hybrid_encode_helpers.go` | 41 |
| `celt/output_helpers.go` | 41 |
| `internal/lpcnetplc/analysis.go` | 40 |
| `celt/recovery_helpers.go` | 38 |
...
- `encoder/encoder.go`: `EncodeFloat32WithAnalysisMaxBytes` now copies public float input directly into `scratchInputPCM []opusRes` and runs analysis from `[]float32`; the public float32 entry point no longer populates a reusable `[]float64` input/analysis bridge. Remaining Opus wrapper scratch debt is the still-needed `scratchInputPCM64` bridge into the unmigrated CELT core. Treat it as wrapper debt, not codec-domain storage to copy into new paths.
...
- `celt/recovery_helpers.go`: periodic PLC now reuses `scratchPLCExc []celtSig` directly for FIR input instead of maintaining a duplicate float32 excitation conversion buffer. Remaining `ensureFloat64Slice` use in recovery helpers, channel adapters, DRED concealment, silence helpers, and state helpers is runtime decode/PLC scratch and must migrate.
...
- Legacy public `encoder/Encoder.Encode`, `EncodeWithAnalysis`, and `EncodeWithAnalysisMaxBytes` still accept `[]float64`; wrapper-only conversion scratch such as `scratchInputPCM64` remains until the CELT core boundary is migrated.
...
- `celtSurroundTrim` now stays in `opusVal32`/`celtGLog` storage through the wrapper and CELT encoder. `celtCVBRBoundScale` is still `float64`; verify the reference type before converting.
...
| A01 | Partial | Public PCM API | Make canonical encode/decode float API `[]float32`; isolate optional `float64` wrappers. Hybrid standalone and multistream decode are now float32; root/CELT/SILK compatibility surfaces and multistream legacy encode remain. | `encoder.go`, `encoder/encoder.go`, `pcm.go`, `multistream*.go`, `celt`, `silk` |
