...
- 2026-05-26 CELT VQ math checkpoint: `exp_rotation()` coefficient generation now mirrors libopus float-build `celt/mathops.h:celt_cos_norm()` by doing the C `double` `cos((.5f*PI)*x)` operation in `internal/opusmath.CELTCosNormF32()` and immediately narrowing back to `float32`. CELT encoder/decoder `sampleRate` config state plus encoder frame/allocation/intensity/spread/tapset decision state is now stored as `int32`, with the control-width reflection guard covering the narrowed fields. This keeps runtime CELT storage and scratch at libopus float/int width, keeps `make test-type-parity` passing with the six source-cited OSCE expression rows, and clears the strict C-backed `TestExpRotationMatchesLibopusFloatPath`, `TestAlgQuantMatchesLibopusFloatPath`, and `TestAlgUnquantMatchesLibopusFloatPath` failures. The focused CELT-FB-20ms-stereo-128k byte lane that this checkpoint chased is now byte-clean (`make test-byte-parity-focus` passes); CELT encode is byte-exact across the CBR matrix.
- 2026-05-26 CELT MDCT math checkpoint: the long-frame forward MDCT mix now uses a float32 arm64 `FMADDS` helper for the libopus-like fused source shape instead of a runtime `float64` bridge. C-backed MDCT tests cover both low-scale and CELT signal-scale inputs, `make test-type-parity` remains at the six source-cited OSCE expression findings, and the focused byte lane improves to `15/51` (`chirp_sweep_v1`) and `9/51` (`am_multisine_v1`) mismatched payloads.
...
- 2026-05-26 CELT amd64/coarse-energy checkpoint: linux/amd64 now selects the libopus x86/SSE-shaped `celt_inner_prod()` order for normalized-vector decisions instead of limiting that path to Windows, while `purego` remains scalar. CELT coarse-energy encode residuals and previous-band updates now preserve `quant_bands.c:quant_coarse_energy_impl()` float expression order (`x - coef*oldE - prev[c]`, then `prev[c] + q - beta*q`) in both range-encode paths. This fixes the local linux/amd64 focused `CELT-FB-20ms-stereo-128k-impulse_train_v1` quality reproduction without weakening fixtures or thresholds; hosted CI later showed the same impulse gap on commit `3d1730ac`, so the remaining real-CI blocker is tracked in the packed PVQ checkpoint below. The remaining linux/amd64 allocation frame `30` mismatch is still tracked as a separate coarse/tell drift.
- 2026-05-26 CELT amd64 pitch-kernel checkpoint: linux/amd64 pitch and prefilter math now follows the libopus x86 float helper dispatch, including SSE-shaped four-lag `xcorr_kernel_sse`, SSE inner/dual-inner reductions, and the AVX2/FMA eight-lag `celt_pitch_xcorr_avx2()` path when CPUID exposes AVX2+FMA. The FMA point is a float32 assembly helper, not a runtime `float64` bridge, and an amd64 unit test guards its ABI. Docker linux/amd64 passes the live libopus pitch/prefilter oracle tests plus `make test-quality`; `make test-type-parity` remains at the six source-cited OSCE C-double expression findings.
- 2026-05-26 CELT amd64 PVQ packed-approx checkpoint: the hosted linux parity run after the pitch patch still showed the unchanged impulse gap, so the x86 PVQ path now mirrors libopus `celt/x86/vq_sse2.c:op_pvq_search_sse2()` more tightly by using packed `RCPPS`/`RSQRTPS` wrappers rather than scalar approximation shims. The projection sum is horizontally reduced and replicated before the reciprocal, matching the SSE source shape; the pulse search computes four `rsqrt` lanes at a time before lane-local max updates and cross-lane tie selection. This removes the scalar approximation bridge without adding widened runtime math. Docker linux/amd64 with a fresh x86_64 libopus build passes the live PVQ/pitch/prefilter oracle tests and the focused byte-parity gate; hosted CI confirmation is pending on the next push.
- 2026-05-26 CELT amd64 PVQ/inner-product SSE checkpoint: the x86 PVQ helper now uses libopus-sized `N+3` `absX`/`y` scratch, sign-bit-cleared `fabsf` behavior, padded sentinel lanes, and an assembly best-id scan mirroring `celt/x86/vq_sse2.c:op_pvq_search_sse2()` count/mask/max behavior instead of a scalar Go tie loop. The dead invalid-sum assignment found by CI lint is gone. The amd64 `celt_inner_prod()` path now routes through an SSE assembly helper matching `celt/x86/pitch_sse.c:celt_inner_prod_sse`, while `purego` and non-amd64 keep the Go libopus-order fallback. This follows the architecture helper's operation shape instead of preserving compatibility wrappers or adding casts.
...
The guard scans runtime Go files for `float64`, `complex128`, `KissFFT64State`, `ensureFloat64Slice`, and `ensureComplexSlice`, then compares the result with `tools/type_parity_allowlist.tsv`. New findings fail. Removed findings also fail until the baseline is refreshed, so cleanup stays visible in review. As of this checkpoint, local `make test-type-parity` passes with six source-cited OSCE C `double` expression rows; these are not runtime widened buffers or compatibility bridges.
...
These are the source-cited `dnn/nndsp.c` C `double` expression parity rows in `internal/osce/bwe/runtime.go` and `internal/osce/lace/runtime.go`; they are immediate expression round-trips back to `float32`, not reusable scratch.
