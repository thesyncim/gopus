...
- A03 CELT core vectors are active and oracle-backed: `aa373dcd` fixes strict CELT VQ oracle cases. Broad runtime vector and scratch migration remains open.
- A07 SILK FLP storage is active and oracle-backed: `e04a55db` links the SILK LPC oracle to the configured libopus archive and `ffa3a0d3` makes that oracle protocol endian-stable. The `silk_float` versus true C `double` storage split remains open.
- A08 fixed math is partial: `8f525312` added C-backed oracle coverage for SILK fixed wrapper primitives and corrected the reversed-bound `silk_LIMIT` behavior for `silkLimitInt`/`silkLimit32`.
...
- A11 oracle/build infrastructure is active: `32f37ba4`, `e04a55db`, `ffa3a0d3`, `458dd69b`, `4c9a5188`, and `aa373dcd` hardened helper cache keys, linked SILK LPC helpers to the configured archive, made SILK oracle protocols endian-stable, stabilized missing-fixture cross-validation behavior, and guarded strict CELT VQ oracle cases against unsupported libopus PVQ table pairs. The current task also tightens the libopus ensure script so a cached tree is not considered current unless it has a host/compiler-matched stamp and `.libs/libopus.a`, moves the CELT QEXT VQ oracle onto the configured QEXT archive instead of compiling default-tree sources with QEXT flags, and extends scalar DNN/OSCE helper stamps to include native OS/arch plus compiler path/target/version while clearing inherited `CC`/`LDFLAGS`.
- SILK stereo byte parity remains active: a fixture-derived packet-0 `StereoLRToMSWithRates` oracle case found and fixed the `silentSideLen` threshold behavior, matching `stereo_LR_to_MS.c` for that routine. The full `SILK-WB-20ms-stereo-48k` `chirp_sweep_v1` encoder fixture still shows 51/51 payload mismatches with matching mode/length histograms, so the remaining blocker is downstream of this routine or in surrounding encoder cadence. Do not mark A09 done until the full encoder path is byte/range-final clean or the next blocker is documented here.
...
- `silk/encoder.go`: `scratchLpcBurg`, `scratchBurgAf`, `scratchBurgCFirstRow`, `scratchBurgCLastRow`, `scratchBurgCAf`, `scratchBurgCAb`, and `scratchBurgResult` may remain `float64` only where they directly mirror `burg_modified_FLP.c` C `double` arrays. Add source comments/tests.
- `silk/encoder.go`: `scratchLtpInput`, `scratchLtpRes`, `scratchPitchWsig`, `scratchPitchAuto`, `scratchPitchRefl`, `scratchPitchA`, `scratchResNrg`, `scratchLpcXF64`, `scratchLpcATmp`, `scratchLpcResidual`, `scratchNlsfCos`, `scratchNlsfP`, and `scratchNlsfQ` need per-function classification. Convert any `silk_float` storage/control scratch to `float32`; keep `float64` only for named C `double` locals.
...
| A01 | Open | Public PCM API | Make canonical encode/decode float API `[]float32`; isolate optional `float64` wrappers. | `encoder.go`, `encoder/encoder.go`, `hybrid/*.go`, `pcm.go`, `multistream*.go` |
...
| A08 | Partial | Fixed math | Build exact fixed helper tests and replace mismatched `int`/overflow arithmetic. | `silk/libopus_fixed.go`, `silk/float_cast.go`, `silk/ltp_quant.go`, `silk/nsq*.go`, `encoder/vad.go` |
