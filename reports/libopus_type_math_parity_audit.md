...
- A07 SILK FLP storage is active and oracle-backed: `e04a55db` links the SILK LPC oracle to the configured libopus archive and `ffa3a0d3` makes that oracle protocol endian-stable. The current follow-up splits LPC/Burg boundaries so persistent `silk_float` storage and FindLPC residual/input scratch use `float32`, while true `burg_modified_FLP.c` C `double` work arrays stay `float64`. More SILK scratch remains open: pitch residual, sparseness input, residual-energy surfaces, gain processing, legacy float stereo predictor helpers, and stale double LPC/LSF helpers must either migrate to `float32`/fixed wrappers or be deleted so accidental reuse cannot reintroduce double-domain parity drift.
...
- A10 extension validation has support work landed: `4c9a5188` made generated DNN model blob headers explicit little-endian through a shared C writer. The A10 runtime conversion goal remains open until OSCE/DRED/LPCNet codec-domain `float64`/`complex128` paths are migrated or source-cited.
- A11 oracle/build infrastructure is active: `32f37ba4`, `e04a55db`, `ffa3a0d3`, `458dd69b`, `4c9a5188`, `aa373dcd`, `24960979`, and `0314e53c` hardened helper cache keys, linked SILK LPC helpers to the configured archive, made SILK oracle protocols endian-stable, stabilized missing-fixture cross-validation behavior, guarded strict CELT VQ oracle cases against unsupported libopus PVQ table pairs, tightened the libopus ensure script so a cached tree is not considered current unless it has a host/compiler-matched stamp and `.libs/libopus.a`, moved the CELT QEXT VQ oracle onto the configured QEXT archive instead of compiling default-tree sources with QEXT flags, and extended scalar DNN/OSCE helper stamps to include native OS/arch plus compiler path/target/version while clearing inherited `CC`/`LDFLAGS`. Direct `FindOrEnsure*` fixture/tool entry points must validate before returning an existing executable; on Windows, where CI builds helpers under MSYS2 and then runs Go tests from PowerShell, validation now invokes `tools/ensure_libopus.sh` from the candidate repo root with a slash-normalized relative script path before falling back to an existing helper. Fallback discovery may use an existing helper only if a parsed v5 native build stamp exactly matches the helper flags, Windows host family, Go arch, compiler target, static archive, `opus_demo`, and `opus_compare`. Tool discovery also searches `GITHUB_WORKSPACE` and the compiled source repo root so package-local test working directories and PowerShell/MSYS path boundaries still land on the same pinned helper tree.
- A11 fixture provenance remains open: CI now has strong native libopus build stamps and rejects stale helper flags in `FindOrEnsure*`, and the Linux container-generated fixture set is now named/selected as `linux_amd64` rather than being treated as generic `amd64` truth on Windows and macOS. Checked-in fixture JSON still lacks deterministic provenance fields (`goos`, `goarch`, host OS/arch, compiler target/version, and libopus build-stamp digest), so do not treat cross-arch generated fixtures as audit-complete until the schema records and validates that data.
- SILK stereo byte parity remains active: `7bd6529f` added a fixture-derived packet-0 `StereoLRToMSWithRates` oracle case and fixed the `silentSideLen` threshold behavior, matching `stereo_LR_to_MS.c` for that routine. Follow-ups removed Go-only inactive-VAD side-channel bit caps, added a C-backed packet-0 wrapper oracle showing TargetRate/MStargetRates/mid `maxBits`/`useCBR`/`condCoding`/`tell` match libopus, and moved frame seed selection to the libopus `Seed = frameCounter++ & 3` cadence. The `SILK-WB-20ms-stereo-48k` `chirp_sweep_v1` fixture still shows `gapQ=-213.20`, `payloadMismatch=51/51`, and `firstPayloadMismatch=0`, so the next blocker is downstream inside the first mid-channel frame-core internals after the packet-0 wrapper checkpoint. The next oracle should assert the packet-0 wrapper's VAD/activity/tilt/SNR fields first, then add a packet-0 mid-channel `silk_encode_frame_FLP` oracle before deeper stage probes. Do not mark A09 done until the full encoder path is byte/range-final clean or the next blocker is documented here.
...
- `celt/scratch.go`: `ensureComplexSlice` returns `[]complex128`. Runtime FFT scratch should use `complex64`/`kissCpx` unless a reference function stores C `double complex`, which libopus CELT does not in the float build.
...
- `encoder/hybrid.go`: `prevHBGain float64` should be `opus_val16`/`float32`.
- `encoder/hybrid.go`: `scratchTransitionPCM`, `scratchBandLogE2`, `scratchAnalysisE`, `scratchPrevEnergy`, `scratchNextEnergy`, `scratchMDCTInput`, `scratchMDCTHist`, `scratchMDCTResult`, `scratchDeintLeft`, and `scratchDeintRight` must match the CELT/Opus types they store.
...
- `celt/decoder_qext_state.go`: `scratchEnergies`, `scratchSpectrumL`, and `scratchSpectrumR` should match QEXT/CELT energy and coefficient domains.
- `celt/decoder_dred_state_enabled.go`: `scratchPLCDREDBase []float64` should match the PLC/DRED PCM domain.
...
- `celt/scratch.go` `bandDecodeScratch`: `left`, `right`, `norm`, `lowband`, `coeffs`, `bandVectors`, `bandVectorsL`, `bandVectorsR`, `bandStorage`, `bandStorageL`, `bandStorageR`, `pvqNorm`, `foldResult`, `hadamardTmp`, and `quantWork` are codec-domain vectors and should not be `float64`.
- `celt/scratch.go` `bandEncodeScratch`: `norm`, `lowbandScratch`, `xSave`, `ySave`, `normSave`, `xResult0`, `yResult0`, `normResult0`, `hadamardTmp`, and `quantWork` are codec-domain vectors and should not be `float64`.
...
- `celt/mdct.go`: `imdctScratch` and IMDCT functions still accept/return `[]float64`; spectrum and overlap scratch must match `celt_sig`/`celt_norm`/`opus_res`.
...
- Current follow-up converted the LPC/Burg boundary: removed transitional `scratchLpcBurg` and `scratchLpcXF64`, changed Burg result storage to `[]float32` (`silk_float`), and changed FindLPC input/residual scratch to `[]float32`. It also removed unused float64 pitch-window/autocorrelation/reflection/LPC scratch fields that no longer participate in runtime analysis.
...
- `silk/encoder.go`: `scratchLtpRes`, `scratchResNrg`, `scratchLpcATmp`, `scratchNlsfCos`, `scratchNlsfP`, and `scratchNlsfQ` still need per-function classification. Convert any `silk_float` storage/control scratch to `float32`; keep `float64` only for named C `double` locals.
- `silk/lpc_analysis.go`, `silk/pitch_residual.go`, and `silk/pred_coefs.go`: `ensureFloat64Slice` use must be justified function by function. A comment like "analysis buffer as float64" is not enough.
...
- `silk/encoder.go` contains many `[]float64` scratch buffers and `float64` state-like fields.
- `pitch_residual.go` returns `[]float64` residuals and stores LPC/autocorrelation helpers in `float64`.
...
| A10 | Open | Extensions | Convert OSCE/DRED/LPCNet codec-domain float64/complex128 to float32 unless source uses C double. | `internal/osce`, `internal/lpcnetplc`, `encoder/dred_runtime.go` |
...
| A13 | Open | Runtime scratch enforcement | Sweep every remaining runtime `scratch` + `float64`/`complex128` match and either migrate it or cite the exact C `double` source. | `celt/scratch.go`, `celt/decoder_types.go`, `encoder/encoder.go`, `encoder/hybrid.go`, `silk/encoder.go`, `multistream/encoder.go` |
