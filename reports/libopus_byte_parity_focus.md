...
2026-05-26 linux/amd64 local checkpoint: amd64 float builds now use the same x86/SSE-shaped `celt_inner_prod()` order as libopus for normalized-vector energy/theta decisions, while `purego` keeps the scalar path. CELT coarse-energy encoding also follows `celt/quant_bands.c:quant_coarse_energy_impl()` float expression order for `f = x - coef*oldE - prev[c]` and the `prev[c] + q - beta*q` update. This fixes the local linux/amd64 focused impulse reproduction for `CELT-FB-20ms-stereo-128k-impulse_train_v1` (`gapQ -53.32`, `payloadMismatch=43/51` -> `gapQ=0.00`, `payloadMismatch=0/51`) and keeps speech clean. Hosted CI later reproduced the same impulse gap on commit `3d1730ac`, so the remaining hosted blocker is tracked in the packed PVQ checkpoint below. On linux/amd64 the focused byte lane is now `35/51` for `chirp_sweep_v1` (first mismatch frame `1`) and `30/51` for `am_multisine_v1` (first mismatch frame `2`). The frame `30` allocation mismatch remains one frame, so the next allocation investigation should stay on the coarse/tell trace rather than changing `clt_compute_allocation()`.

2026-05-26 amd64 pitch-kernel checkpoint: CELT pitch search now mirrors libopus x86 float dispatch for `celt/x86/pitch_sse.c:{xcorr_kernel_sse,celt_inner_prod_sse,dual_inner_prod_sse}` and `celt/x86/pitch_avx.c:celt_pitch_xcorr_avx2()`. The AVX2/FMA path is gated by runtime CPUID and uses a float32 FMA assembly helper; it does not add a `float64` math bridge. A dedicated amd64 test locks the helper return ABI after the initial zero-return bug was caught locally. Docker linux/amd64 with AVX2/FMA exposed passes `make test-quality`, the focused `make test-byte-parity-focus`, and the live libopus prefilter/pitch oracle tests. Focused linux/amd64 results remain `35/51` for `chirp_sweep_v1`, `30/51` for `am_multisine_v1`, and byte-clean for `speech_like_v1` and `impulse_train_v1`, matching the native libopus quality envelope in local reproduction.

2026-05-26 amd64 PVQ packed-approx checkpoint: hosted `test-linux-parity` on commit `3d1730ac` still failed `CELT-FB-20ms-stereo-128k-impulse_train_v1` with the unchanged real-hardware gap (`goQ=-489.93`, `libQ=-436.61`, `gapQ=-53.32`, `payloadMismatch=43/51`), so the pitch kernel was not the hosted blocker. The x86 PVQ helper now calls packed `RCPPS`/`RSQRTPS` wrappers at the same points as `celt/x86/vq_sse2.c:op_pvq_search_sse2()` instead of scalar `RCPSS`/`RSQRTSS` substitutes, while keeping the SSE horizontal sum replicated across all four lanes before the projection reciprocal. Docker linux/amd64 with an isolated x86_64 libopus build passes the live PVQ/pitch/prefilter oracle tests and `make test-byte-parity-focus`; focused linux/amd64 remains `35/51` for `chirp_sweep_v1`, `30/51` for `am_multisine_v1`, and byte-clean for `speech_like_v1` and `impulse_train_v1`. CI confirmation is pending on the next push.

2026-05-26 amd64 PVQ/inner-product SSE checkpoint: hosted `test-linux-parity` on commit `c601ba05` still reported the same impulse-train quality gap when using stale checked-in fixtures, while `lint-static-analysis` also caught an ineffectual assignment in the x86 PVQ invalid-sum fallback. The CI quality lane now regenerates platform fixtures with the runner's own libopus build (`make fixtures-gen-platform`) before `GOPUS_REQUIRE_PLATFORM_FIXTURES=1 make test-quality`, so the quality comparison cannot silently fall back to fixtures generated on a different architecture. The x86 PVQ path now also allocates the libopus `N+3` scratch lanes, clears the sign bit for `fabsf` parity, writes the `-100`/`100` tail sentinels, removes the dead fallback assignment, and selects the pulse target through an SSE2 best-id helper shaped after `celt/x86/vq_sse2.c:op_pvq_search_sse2()` (`CMPPS`/`PMAXSW` tie behavior included). On amd64, CELT normalized inner products now have an SSE assembly implementation matching `celt/x86/pitch_sse.c:celt_inner_prod_sse`, with a bitwise asm-vs-libopus-order unit test. Local darwin/arm64 `make test-byte-parity-focus` remains green with `13/51` chirp and `8/51` AM payload mismatches, and Docker linux/amd64 passes the focused CELT/PVQ/inner-product oracle set.
