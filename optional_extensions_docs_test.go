...
func TestOptionalExtensionDocsContract(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"Hybrid fullband 20 ms mono carried-payload-only seam",
		...
		"48 kHz CELT cached stereo first-loss live-sequence seam",
		"Hybrid mono 20/40 ms packet-envelope exactness and Hybrid stereo 40 ms packet-envelope/primary-frame exactness remain diagnostic",
	...
	for _, needle := range []string{
		...
		"selected 16 kHz Hybrid mono live-sequence seams",
		"48 kHz CELT cached",
		"stereo first-loss live-sequence seam",
