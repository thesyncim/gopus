...
func TestOptionalExtensionDocsContract(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"standalone DRED wrapper lifecycle/no-allocation and libopus parse/decode checks",
		"`make test-dred-tag`",
	...
	for _, needle := range []string{
		...
		"zero-allocation, and libopus parse/decode coverage plus the supported-tag narrow",
