...
func TestOptionalExtensionDocsContract(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"multistream decoder DRED still cache/timing-only",
		...
		"multistream neural DRED decoder support remains unsupported/open",
		"`make test-dred-tag`",
	...
	for _, needle := range []string{
		...
		"multistream decoder DRED remains cache/timing-only",
