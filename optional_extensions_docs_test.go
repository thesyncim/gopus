...
func TestOptionalExtensionDocsContract(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"selected 48 kHz CELT stereo cached/live first/second-loss and next-packet handoff seams",
		"explicit first-loss and recovery lifecycle/cursor seams, and the 48 kHz SILK WB explicit stereo first-loss seam",
		"selected 16 kHz CELT/Hybrid stereo explicit first-loss probes",
		"48 kHz CELT elementary-stream multistream DRED consumer",
		...
		"broader multistream Hybrid/SILK neural DRED packet/mode coverage remains seam-specific",
		"`make test-dred-tag`",
	...
	for _, needle := range []string{
		...
		"single-coupled multistream SILK/CELT/Hybrid 20 ms stereo DRED",
		"`Encode` remains",
		...
		"OSCE BWE/LACE numerical forward-pass and runtime integration coverage",
		"bootstrap coverage, the required mono decoder explicit/live numerical",
		"matrix",
		"selected 16 kHz Hybrid mono live-sequence seams",
		"selected 48 kHz CELT stereo cached/live first/second-loss and",
		"next-packet handoff seams",
		"48 kHz SILK WB explicit",
		"first-loss seam are parity-gated",
		"selected 16 kHz CELT/Hybrid stereo explicit first-loss",
		"broader multistream Hybrid/SILK DRED remains seam-specific",
